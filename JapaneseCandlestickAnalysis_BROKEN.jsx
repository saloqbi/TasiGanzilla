...
// ⚡ SIMPLIFIED ULTRA-FAST THROTTLE: Optimized for maximum performance
// 🚀 دالة throttle مبسطة وفائقة السرعة للأداء الأمثل
function throttle(func, limit = 4) { // تقليل إلى 4ms فقط للحصول على ~240fps
  let lastCall = 0;
...
// 🌐 ARABIC TEXT FIX: Enhanced Arabic text rendering for Canvas
// 🔤 دالة محسنة لإصلاح عرض النصوص العربية في Canvas
function setArabicFont(ctx, size = 12, weight = 'normal') {
  // 🔤 قائمة خطوط محسنة تدعم النصوص العربية بشكل أفضل
  const arabicFonts = [
    'Noto Sans Arabic',
    'Segoe UI',
    'Tahoma', 
  ...
  // تطبيق الخط مع تحسينات النصوص العربية
  ctx.font = `${weight} ${size}px ${arabicFonts.join(', ')}`;
  ctx.textBaseline = 'middle';
  ctx.textAlign = 'center';
  
  // 🌐 تحسينات إضافية لعرض النصوص العربية
  ctx.direction = 'rtl';
...
// 🔤 دالة مساعدة لعرض النص العربي بطريقة محسنة
function drawArabicText(ctx, text, x, y, maxWidth = null) {
  // حفظ إعدادات السياق الحالية
  ctx.save();
  
  // تطبيق تحسينات النصوص العربية
  ctx.direction = 'rtl';
  ctx.textBaseline = 'middle';
  
  // رسم النص مع أو بدون عرض أقصى
  if (maxWidth) {
...
const JapaneseCandlestickContent = React.forwardRef((props, ref) => {
  ...
  // 🔍 متغيرات التكبير والتصغير التفاعلية
  const [zoomState, setZoomState] = useState({
    timeZoom: 1, // تكبير المحور الزمني
    priceZoom: 1, // تكبير المحور السعري
    timeOffset: 0, // إزاحة أفقية
    priceOffset: 0, // إزاحة رأسية
    mousePosition: { x: 0, y: 0 }, // موقع الماوس للتركيز
    isZooming: false, // حالة التكبير النشط
    zoomFocus: null // نقطة التركيز للتكبير
  });

  // 🎯 إعدادات التكبير الافتراضية
  const ZOOM_CONFIG = {
    minTimeZoom: 0.1, // أقل تكبير زمني
    maxTimeZoom: 10, // أكبر تكبير زمني
    minPriceZoom: 0.5, // أقل تكبير سعري
    maxPriceZoom: 5, // أكبر تكبير سعري
    wheelSensitivity: 0.05, // تقليل حساسية عجلة الماوس
    smoothZoom: true, // تكبير سلس
    zoomAnimationDuration: 150 // تقليل مدة انيميشن التكبير
  };
  ...
  const drawSmartMovingCandlesticks = (canvas, ctx, data) => {
  ...
  // ⚡ ULTRA-FAST ZOOM: Revolutionary Wheel Zoom Handler with Maximum Performance
  const handleWheelZoom = useCallback((event) => {
    event.preventDefault();
    
    // � تحسين جذري: تقليل throttling إلى الحد الأدنى للحصول على استجابة فورية
    const now = Date.now();
    if (now - lastZoomTimeRef.current < 8) { // تقليل من 16ms إلى 8ms
      return;
    }
    ...
    // 🚀 تحسين فائق: استخدام requestAnimationFrame للحصول على أقصى سلاسة
    requestAnimationFrame(() => {
      const rect = canvas.getBoundingClientRect();
      const mouseX = event.clientX - rect.left;
      const mouseY = event.clientY - rect.top;
      
      // تحديد نوع التكبير حسب المفتاح المضغوط
      const isCtrlPressed = event.ctrlKey || event.metaKey;
      const isShiftPressed = event.shiftKey;
      
      // 🔥 تحسين السرعة: زيادة حساسية الزوم لاستجابة أسرع
      let delta = event.deltaY > 0 ? -1 : 1;
      delta *= ZOOM_CONFIG.wheelSensitivity * 1.5; // زيادة 50% في الحساسية
      
      setZoomState(prevState => {
        let newTimeZoom = prevState.timeZoom;
        let newPriceZoom = prevState.priceZoom;
        let newTimeOffset = prevState.timeOffset;
        let newPriceOffset = prevState.priceOffset;
        
        if (isCtrlPressed) {
          // تكبير/تصغير المحور الزمني
          newTimeZoom = Math.max(
            ZOOM_CONFIG.minTimeZoom,
            Math.min(ZOOM_CONFIG.maxTimeZoom, prevState.timeZoom + delta)
          );
          
          // 🚀 حساب محسن للإزاحة الأفقية
          const canvasWidth = canvas.width;
          const zoomRatio = newTimeZoom / prevState.timeZoom;
          newTimeOffset = mouseX - (mouseX - prevState.timeOffset) * zoomRatio;
          
        } else if (isShiftPressed) {
          // تكبير/تصغير المحور السعري
          newPriceZoom = Math.max(
            ZOOM_CONFIG.minPriceZoom,
            Math.min(ZOOM_CONFIG.maxPriceZoom, prevState.priceZoom + delta)
          );
          
          // 🚀 حساب محسن للإزاحة الرأسية
          const canvasHeight = canvas.height;
          const zoomRatio = newPriceZoom / prevState.priceZoom;
          newPriceOffset = mouseY - (mouseY - prevState.priceOffset) * zoomRatio;
          
        } else {
          // التكبير المزدوج (الزمني والسعري معاً) - محسن للأداء
          newTimeZoom = Math.max(
            ZOOM_CONFIG.minTimeZoom,
            Math.min(ZOOM_CONFIG.maxTimeZoom, prevState.timeZoom + delta)
          );
          ...
          // 🔥 حساب محسن للإزاحات مع تحسين الأداء
          const canvasWidth = canvas.width;
          const canvasHeight = canvas.height;
          const timeZoomRatio = newTimeZoom / prevState.timeZoom;
          const priceZoomRatio = newPriceZoom / prevState.priceZoom;
          
          newTimeOffset = mouseX - (mouseX - prevState.timeOffset) * timeZoomRatio;
          newPriceOffset = mouseY - (mouseY - prevState.priceOffset) * priceZoomRatio;
        }
        ...
        return {
          ...
          isZooming: true,
          zoomFocus: { x: mouseX, y: mouseY }
        };
      });
      
      // ⚡ إلغاء إعادة الرسم المتأخرة واستخدام رسم فوري محسن
      if (redrawTimeoutRef.current) {
        clearTimeout(redrawTimeoutRef.current);
      }
      
      // 🚀 رسم فوري بدون تأخير لاستجابة لحظية
      if (candleData.length > 0) {
        const ctx = canvas.getContext('2d');
        // إعادة الرسم ستحدث تلقائياً عبر useEffect المحسن
      }
    });
  }, [candleData]);

  // 🎯 دالة إعادة تعيين التكبير
  const resetZoom = useCallback(() => {
    ...
    // 🚀 إعادة رسم الشارت مع تحسين الأداء
    const canvas = canvasRef.current;
    if (canvas && candleData.length > 0 && !isRedrawing) {
      setIsRedrawing(true);
      
      // إلغاء الـ timeout السابق إذا كان موجود
      if (redrawTimeoutRef.current) {
        clearTimeout(redrawTimeoutRef.current);
      }
      
      // تأخير إعادة الرسم لتحسين الأداء
  ...
  }, [candleData, isRedrawing]);

  // 🖱️ دالة التحريك الأفقي (Pan)
  const handlePan = useCallback((event) => {
    if (!zoomState.isZooming) return;
    
    // 🚀 تطبيق throttling لتحسين الأداء
    const now = Date.now();
    ...
    setZoomState(prevState => ({
      ...prevState,
      timeOffset: prevState.timeOffset + (mouseX - prevState.mousePosition.x),
      priceOffset: prevState.priceOffset + (mouseY - prevState.mousePosition.y),
      mousePosition: { x: mouseX, y: mouseY }
    }));
    
    // 🚀 إعادة رسم الشارت مع debouncing لتحسين الأداء
    if (candleData.length > 0 && !isRedrawing) {
      setIsRedrawing(true);
  ...
  }, [zoomState.isZooming, candleData, isRedrawing]);

  // 🎯 دالة التركيز على نقطة محددة
  const focusOnPoint = useCallback((x, y) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    
    setZoomState(prevState => ({
      ...prevState,
      timeOffset: x - canvas.width / 2,
      priceOffset: y - canvas.height / 2,
      mousePosition: { x, y },
      zoomFocus: { x, y }
    }));
  ...
  // 🧹 دالة تنظيف عند إلغاء تحميل المكون
  useEffect(() => {
    return () => {
      if (redrawTimeoutRef.current) {
        clearTimeout(redrawTimeoutRef.current);
  ...
  const handleZoom = useCallback((factor) => {
    ...
    setZoomState(prevState => {
      const newTimeZoom = Math.max(
        ZOOM_CONFIG.minTimeZoom,
        Math.min(ZOOM_CONFIG.maxTimeZoom, prevState.timeZoom * factor)
      );
      const newPriceZoom = Math.max(
        ZOOM_CONFIG.minPriceZoom,
        Math.min(ZOOM_CONFIG.maxPriceZoom, prevState.priceZoom * factor)
      );
      
      const timeZoomRatio = newTimeZoom / prevState.timeZoom;
      const priceZoomRatio = newPriceZoom / prevState.priceZoom;
      
      const newTimeOffset = centerX - (centerX - prevState.timeOffset) * timeZoomRatio;
      const newPriceOffset = centerY - (centerY - prevState.priceOffset) * priceZoomRatio;
  ...
  // 🖱️ معالج الضغط على الماوس الأوسط للتحريك
  const handleMouseDown = useCallback((event) => {
    if (event.button === 1) { // الماوس الأوسط
      event.preventDefault();
      setZoomState(prev => ({ ...prev, isZooming: true }));
    }
  }, []);

  // 🖱️ معالج تحرير الماوس الأوسط
  ...
  const handleMouseMove = useCallback((event) => {
    // 🚀 تطبيق throttling محسن لتحسين الأداء
    const now = Date.now();
    if (now - lastZoomTimeRef.current < ZOOM_THROTTLE_MS * 2) { // زيادة الفترة
      return;
    }
    lastZoomTimeRef.current = now;
    ...
    // تحديث موضع الماوس فقط إذا تغير بشكل كبير
    setMousePosition(prev => {
      const deltaX = Math.abs(prev.x - x);
      const deltaY = Math.abs(prev.y - y);
      if (deltaX > 5 || deltaY > 5) { // تحديث فقط إذا تغير أكثر من 5 بكسل
        return { x, y };
      }
      return prev;
    });
    
    // 🖱️ التحريك الأفقي عند الضغط على الماوس الأوسط
    if (zoomState.isZooming && event.buttons === 4) { // الماوس الأوسط مضغوط
      handlePan(event);
    }
    
    // إضافة معاينة مباشرة أثناء الرسم مع throttling
    if (isDrawing && drawingStart && (interactiveMode === 'fibonacci' || interactiveMode === 'support_resistance')) {
      ...
      // 🚀 رسم فوري لخط المعاينة مع throttling
      if (interactiveMode === 'fibonacci' && !isRedrawing) {
        setIsRedrawing(true);
        if (redrawTimeoutRef.current) {
          clearTimeout(redrawTimeoutRef.current);
        }
  ...
  }, [candleData, isRedrawing, interactiveMode, isDrawing, drawingStart, zoomState.isZooming, handlePan]);
  ...
  // 🔧 إضافة تسجيل للتشخيص
  useEffect(() => {
    console.log(`📊 حالة البيانات: ${candleData.length} شمعة`);
    if (candleData.length > 0) {
      console.log('📈 أول شمعة:', candleData[0]);
      console.log('📉 آخر شمعة:', candleData[candleData.length - 1]);
    }
  }, [candleData]);
  ...
  //  🕯️ دالة رسم الشموع اليابانية مع دعم التكبير والتصغير
  const drawCandlestickChart = useCallback((canvas, ctx, data) => {
    if (!data || data.length === 0) return;

    // � ضبط أحجام الكانفاس بشكل صحيح
    canvas.width = canvas.offsetWidth;
    canvas.height = canvas.offsetHeight;

    // �🔍 تطبيق إعدادات التكبير والتصغير
    const { timeZoom, priceZoom, timeOffset, priceOffset } = zoomState;
    
    const padding = 60;
    let chartWidth = canvas.width - 2 * padding;
    let chartHeight = canvas.height - 2 * padding;
    
    // تطبيق التكبير على أبعاد الشارت
    chartWidth *= timeZoom;
    chartHeight *= priceZoom;
    
    // 🚀 تحسين الأداء: حساب الأسعار مرة واحدة فقط
    ...
    // 🚀 تحسين الأداء: حساب إعدادات الشموع مرة واحدة
    ...
    // مسح الكانفس - مبسط للأداء الأمثل
    ctx.fillStyle = '#1A1A1A';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // 🔧 تسجيل مبسط للتشخيص (فقط عند الحاجة)
    if (data.length % 10 === 0) {
      console.log(`🎨 رسم ${data.length} شمعة`);
    }

    // رسم شبكة مبسطة للأداء الأمثل
    ctx.strokeStyle = 'rgba(255, 255, 255, 0.1)';
    ctx.lineWidth = 1;
    
    // خطوط أفقية مبسطة (5 خطوط فقط)
    const priceLines = 5;
    ...
    // 🎯 رسم الشموع مبسط للأداء الأمثل
    data.forEach((candle, index) => {
      // حساب المواقع الأساسية
      ...
      const candleColor = isGreen ? '#00FF41' : '#FF1744';
    ...
    console.log(`� رسم ${data.length} شمعة بنجاح`);
  }, []);
      
  
  // 📊 دالة رسم المنطقة (Area Chart) - مبسطة للأداء الأمثل
  ...
  // 🎯 INITIAL DATA GENERATION: إنشاء البيانات التجريبية عند بدء التطبيق
  useEffect(() => {
    // إنشاء بيانات تجريبية فقط إذا لم توجد بيانات ولم يتم اختيار رمز معين
    if ((!candleData || candleData.length === 0) && !selectedSymbol) {
      console.log('🚀 إنشاء بيانات تجريبية...');
      const demoData = generateSmartMovingCandleData();
      setCandleData(demoData);
      
      // تحديث السعر الحالي من آخر شمعة
      if (demoData.length > 0) {
        ...
        if (demoData.length > 1) {
          const prevCandle = demoData[demoData.length - 2];
  ...
  // ⚡ PERFORMANCE OPTIMIZATION: Removed duplicate useEffect to prevent rendering conflicts
  // تم حذف useEffect المتداخل لمنع تداخل عمليات الرسم وتحسين الأداء
  ...
  // 🚀 ULTRA-FAST ZOOM OPTIMIZATION: Optimized Render Effect
  // تحسين جذري لسرعة الزوم - دمج جميع عمليات الرسم في useEffect واحد محسن
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    if (!candleData || candleData.length === 0) {
      const demoData = generateSmartMovingCandleData();
      setCandleData(demoData);
      return; // خروج لإعطاء فرصة لإعادة التشغيل مع البيانات الجديدة
    }

    // �🔥 دالة رسم محسّنة فائقة السرعة مع throttling مطور خصيصاً للزوم
    const renderEverything = () => {
      if (!candleData.length) return;
      
      const ctx = canvas.getContext('2d');
      
      // 🚀 تحسين الأداء: استخدام requestAnimationFrame لرسم أكثر سلاسة
      requestAnimationFrame(() => {
        ...
        // 🚀 تحسين الأداء: تجميع عمليات الرسم الإضافية في دفعات
        const renderBatch = [];
        ...
        // 🚀 تحسين الأداء: تنفيذ جميع العناصر الإضافية في دفعة واحدة محسنة
        renderBatch.forEach(element => element());
        
        // رسم وضع التفاعل إذا كان مفعلاً
        if (interactiveMode && interactiveMode !== 'none') {
          ...
          if (modeConfig) {
            ...
            // 🌐 رسم نص المؤشر باستخدام الدالة المحسنة للنصوص العربية
            ctx.fillStyle = '#FFFFFF';
            ctx.globalAlpha = 1;
            setArabicFont(ctx, 12, 'bold');
            drawArabicText(ctx, `وضع: ${modeConfig.name}`, canvas.width - 80, 25);
            ctx.restore();
          }
        }
      });
  ...
  useEffect(() => {
    ...
    canvas.addEventListener('contextmenu', (e) => e.preventDefault()); // منع القائمة اليمنى
  ...
  }, [candleData, interactiveMode, isDrawing, drawingStart, detectCandlestickPatterns, previewData, handleWheelZoom, handleMouseDown, handleMouseUp, handleMouseLeave, handleMouseMove, handleCanvasClick]);
  // إعدادات النمط
  const styles = {
    container: {
      width: '100%',
      ...
      <div style={styles.interactiveToolbar}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ color: '#FFD700', fontSize: '12px', fontWeight: 'bold' }}>🔍 التكبير</span>
            <div style={{ display: 'flex', gap: '4px' }}>
              <button 
                onClick={() => {
                  setZoomState(prev => ({
                    ...prev,
                    timeZoom: Math.max(ZOOM_CONFIG.minTimeZoom, prev.timeZoom - 0.2),
                    priceZoom: Math.max(ZOOM_CONFIG.minPriceZoom, prev.priceZoom - 0.2)
                  }));
                }}
                style={{
                  background: '#FFD700',
              ...
              <button 
                onClick={() => {
                  setZoomState(prev => ({
                    ...prev,
                    timeZoom: Math.min(ZOOM_CONFIG.maxTimeZoom, prev.timeZoom + 0.2),
                    priceZoom: Math.min(ZOOM_CONFIG.maxPriceZoom, prev.priceZoom + 0.2)
                  }));
                }}
                style={{
                  background: '#FFD700',
                ...
                title="تكبير (Ctrl + عجلة الماوس)"
              >
                🔍+
              </button>
            </div>
            <span style={{ color: '#B0BEC5', fontSize: '10px' }}>
              زمني: {zoomState.timeZoom.toFixed(1)}x | سعري: {zoomState.priceZoom.toFixed(1)}x
            </span>
          </div>
        </div>
