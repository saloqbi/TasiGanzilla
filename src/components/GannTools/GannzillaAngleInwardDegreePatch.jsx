...
const PATCH_CANVAS_ID = 'gannzilla-angle-inward-degree-patch-v12';
const SOURCE_CANVAS_ID = 'gannzilla-long-number-digital-renderer-v1';
const TWO_PI = Math.PI * 2;
const ANGLE_FONT_STACK = 'Arial Black, Trebuchet MS, Tahoma, Arial, sans-serif';
const MARKER = 'CELL_ANGLE_INWARD_WITH_DEGREE_V12';
...
function drawAngleText(ctx, text, x, y, fontSize, color) {
  ctx.save();
  ctx.translate(Math.round(x) + 0.5, Math.round(y) + 0.5);
  ctx.font = `800 ${fontSize}px ${ANGLE_FONT_STACK}`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.lineJoin = 'round';
  ctx.lineWidth = Math.max(0.42, fontSize * 0.035);
...
function eraseOldAngle(ctx, x, y, fontSize, fillStyle) {
  const w = Math.max(18, fontSize * 2.65);
  const h = Math.max(13, fontSize * 1.75);
  ctx.save();
...
function drawPatch(overlay, sourceCanvas) {
  ...
  for (let ring = 1; ring <= levels; ring += 1) {
    ...
    for (let i = 0; i < divisions; i += 1) {
      ...
      eraseOldAngle(ctx, oldAngleX, oldAngleY, oldAngleSize, bandFill);

      // Move the angle down/inward away from the circular frame, but keep it above the main number.
      const angleSize = clamp(fs * 0.72, 8.8, 13.8);
      const angleOuterOffset = clamp(metrics.width * 0.24, 5.8, metrics.width * 0.34);
      const angleX = p.x - ux * angleOuterOffset;
      const angleY = p.y - uy * angleOuterOffset;

      drawAngleText(ctx, `${angleSequenceValue(value)}°`, angleX, angleY, angleSize, color);
    }
  }
}

export default function GannzillaAngleInwardDegreePatch() {
  React.useEffect(() => {
    ...
    window.__gannzillaAngleInwardDegreePatchV12 = MARKER;
