...
const BUILD = 593;
const STATE_KEY = '__gannzillaCenterLogoResponsive71V593';
const IMAGE_ID = 'gannzilla-center-logo-responsive-71-v593';
const STYLE_ID = 'gannzilla-center-logo-responsive-71-style-v593';
const STAGE_DATASET_KEY = 'gannzillaCenterLogoResponsive71V593';
...
function ensureStyle() {
  ...
  style.textContent = `
    [data-gannzilla-center-logo-responsive71-v593="true"] {
...
function ensureImage(stage) {
  ...
  if (!(image instanceof HTMLImageElement)) {
    ...
    image.addEventListener('error', () => {
      image.style.setProperty('visibility', 'hidden', 'important');
      image.style.setProperty('opacity', '0', 'important');
      image.dataset.gannzillaCenterLogoImageErrorV593 = 'true';
    }, { once: true });
  }
...
function apply(source = 'apply') {
  ...
  image.dataset.gannzillaCenterLogoResponsive71V593 = 'true';
  image.dataset.gannzillaCenterLogoBuildV593 = String(BUILD);
  image.dataset.gannzillaCenterLogoScaleV593 = String(LOGO_SCALE);
  image.dataset.gannzillaCenterLogoViewportScaleV593 = viewportScale.toFixed(6);
  image.dataset.gannzillaCenterLogoEmptyRadiusV593 = emptyRadiusOnScreen.toFixed(3);
  image.dataset.gannzillaCenterLogoDiameterV593 = diameter.toFixed(3);
  image.dataset.gannzillaCenterLogoInteractiveV593 = 'false';
  image.dataset.gannzillaCenterLogoFixedEffectV593 = 'false';
  image.dataset.gannzillaCenterLogoPanAddedV593 = 'false';
...
function install() {
  ...
  window.GANNZILLA_CENTER_LOGO_RESPONSIVE_71_V593 = true;
  window.__auditGannzillaCenterLogoResponsive71V593 = () => {
    const image = document.getElementById(IMAGE_ID);
    const canvas = findWheel();
    const imageRect = image?.getBoundingClientRect();
    const emptyRadius = Number(image?.dataset.gannzillaCenterLogoEmptyRadiusV593 || 0);
    const logoDiameter = Number(image?.dataset.gannzillaCenterLogoDiameterV593 || 0);
    return {
