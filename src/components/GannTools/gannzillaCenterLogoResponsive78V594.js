...
const BUILD = 594;
const STATE_KEY = '__gannzillaCenterLogoResponsive78V594';
const IMAGE_ID = 'gannzilla-center-logo-responsive-78-v594';
const STYLE_ID = 'gannzilla-center-logo-responsive-78-style-v594';
const STAGE_DATASET_KEY = 'gannzillaCenterLogoResponsive78V594';
...
function ensureStyle() {
  ...
  style.textContent = `
    [data-gannzilla-center-logo-responsive78-v594="true"] {
...
function ensureImage(stage) {
  ...
  if (!(image instanceof HTMLImageElement)) {
    ...
    image.addEventListener('error', () => {
      image.style.setProperty('visibility', 'hidden', 'important');
      image.style.setProperty('opacity', '0', 'important');
      image.dataset.gannzillaCenterLogoImageErrorV594 = 'true';
    }, { once: true });
  }
...
function apply(source = 'apply') {
  ...
  image.dataset.gannzillaCenterLogoResponsive78V594 = 'true';
  image.dataset.gannzillaCenterLogoBuildV594 = String(BUILD);
  image.dataset.gannzillaCenterLogoScaleV594 = String(LOGO_SCALE);
  image.dataset.gannzillaCenterLogoViewportScaleV594 = viewportScale.toFixed(6);
  image.dataset.gannzillaCenterLogoEmptyRadiusV594 = emptyRadiusOnScreen.toFixed(3);
  image.dataset.gannzillaCenterLogoDiameterV594 = diameter.toFixed(3);
  image.dataset.gannzillaCenterLogoInteractiveV594 = 'false';
  image.dataset.gannzillaCenterLogoPanAddedV594 = 'false';
...
function install() {
  ...
  window.GANNZILLA_CENTER_LOGO_RESPONSIVE_78_V594 = true;
  window.__auditGannzillaCenterLogoResponsive78V594 = () => {
    const image = document.getElementById(IMAGE_ID);
    const canvas = findWheel();
    const imageRect = image?.getBoundingClientRect();
    const emptyRadius = Number(image?.dataset.gannzillaCenterLogoEmptyRadiusV594 || 0);
    const logoDiameter = Number(image?.dataset.gannzillaCenterLogoDiameterV594 || 0);
    return {
