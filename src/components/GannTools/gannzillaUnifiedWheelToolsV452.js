const BUILD = 452;
const CONTROL_ID = 'gannzilla-unified-wheel-tools-v452';
const EYE_ID = 'gannzilla-unified-eye-v452';
const MOVE_WRAP_ID = 'gannzilla-unified-move-wrap-v452';
const MOVE_ID = 'gannzilla-unified-move-v452';
const PAD_ID = 'gannzilla-unified-move-pad-v452';
const ZOOM_OUT_ID = 'gannzilla-unified-zoom-out-v452';
const ZOOM_SELECT_ID = 'gannzilla-unified-zoom-select-v452';
const ZOOM_IN_ID = 'gannzilla-unified-zoom-in-v452';
const FULLSCREEN_ID = 'gannzilla-unified-fullscreen-v452';
const STYLE_ID = 'gannzilla-unified-wheel-tools-style-v452';
const STATE_KEY = '__gannzillaUnifiedWheelToolsV452';
...
const ZOOM_STORAGE_KEY = 'tasi-gannzilla-wheel-zoom-v452';
...
function findWheelCanvas() {
  ...
  const preferred = document.querySelector([
    ...
    'canvas[data-gannzilla-unified-wheel-tools-v452="true"]',
  ].join(','));
...
function bindBaseCanvas(canvas) {
  if (!(canvas instanceof HTMLCanvasElement)) return false;
  if (baseCanvas === canvas && baseWidth > 0 && baseHeight > 0) return true;

  canvasObserver?.disconnect();
  baseCanvas = canvas;
  const ratio = Math.max(0.01, currentZoom / 100);
  const rect = canvas.getBoundingClientRect();
  const cssWidth = parseFloat(canvas.style.width) || rect.width || canvas.width;
  const cssHeight = parseFloat(canvas.style.height) || rect.height || canvas.height;
  baseWidth = Math.max(1, cssWidth / ratio);
  baseHeight = Math.max(1, cssHeight / ratio);

  if (typeof MutationObserver === 'function') {
    canvasObserver = new MutationObserver(() => {
      if (applyingCanvasStyle) return;
      window.requestAnimationFrame(() => applyCanvasState('canvas-mutation'));
    });
...
function applyCanvasState(source = 'refresh') {
  const canvas = findWheelCanvas();
  if (!(canvas instanceof HTMLCanvasElement) || !bindBaseCanvas(canvas)) return false;

  applyingCanvasStyle = true;
  ...
  canvas.style.setProperty(
    'transform',
    `translate3d(${Math.round(currentOffset.x)}px, ${Math.round(currentOffset.y)}px, 0)`,
    'important',
  );
  ...
  canvas.dataset.gannzillaUnifiedWheelToolsV452 = 'true';
  canvas.dataset.gannzillaUnifiedZoomV452 = String(currentZoom);
  canvas.dataset.gannzillaUnifiedVisibleV452 = wheelVisible ? 'true' : 'false';
  canvas.dataset.gannzillaPanX = String(Math.round(currentOffset.x));
  canvas.dataset.gannzillaPanY = String(Math.round(currentOffset.y));
  if (wheelVisible) canvas.removeAttribute('aria-hidden');
  else canvas.setAttribute('aria-hidden', 'true');

  window.requestAnimationFrame(() => { applyingCanvasStyle = false; });
  window.dispatchEvent(new CustomEvent('gannzilla:unified-wheel-tools-v452', {
    detail: {
      source,
...
function scheduleCanvasState(source) {
  applyCanvasState(source);
  window.requestAnimationFrame(() => applyCanvasState(`${source}:frame`));
  window.setTimeout(() => applyCanvasState(`${source}:40ms`), 40);
  window.setTimeout(() => applyCanvasState(`${source}:140ms`), 140);
}
...
function installStyle() {
  ...
  style.textContent = `
    ...
    #${EYE_ID}:hover, #${MOVE_ID}:hover, #${ZOOM_OUT_ID}:hover, #${ZOOM_IN_ID}:hover, #${FULLSCREEN_ID}:hover,
    #${MOVE_WRAP_ID}[data-open="true"] #${MOVE_ID} {
      background: linear-gradient(#ffffff, #dcecff) !important;
      border-color: #477da8 !important;
    }

    #${EYE_ID} svg, #${MOVE_ID} svg, #${ZOOM_OUT_ID} svg, #${ZOOM_IN_ID} svg, #${FULLSCREEN_ID} svg {
    ...
    #${PAD_ID} .gz452-pad:hover { background: #dcecff !important; border-color: #6b9cbd !important; }
    #${PAD_ID} .gz452-pad svg { width: 20px !important; height: 20px !important; display: block !important; pointer-events: none !important; }
    #${PAD_ID} .gz452-up { grid-column: 2 !important; grid-row: 1 !important; }
    #${PAD_ID} .gz452-left { grid-column: 1 !important; grid-row: 2 !important; }
    #${PAD_ID} .gz452-center { grid-column: 2 !important; grid-row: 2 !important; }
    #${PAD_ID} .gz452-right { grid-column: 3 !important; grid-row: 2 !important; }
    #${PAD_ID} .gz452-down { grid-column: 2 !important; grid-row: 3 !important; }
  `;
}
...
function moveWheel(direction, source) {
  const step = Math.round(numberParam('wheelMoveStep', 48, 8, 240));
  const next = {
    left: { x: currentOffset.x - step, y: currentOffset.y },
    right: { x: currentOffset.x + step, y: currentOffset.y },
    up: { x: currentOffset.x, y: currentOffset.y - step },
    down: { x: currentOffset.x, y: currentOffset.y + step },
    center: { x: 0, y: 0 },
  }[direction];
  if (!next) return;

  currentOffset = next;
  persistOffset(currentOffset);
  window.dispatchEvent(new CustomEvent('gannzilla:wheel-pan-offset-v305', { detail: { ...currentOffset, source, build: BUILD } }));
  window.dispatchEvent(new CustomEvent('gannzilla:page-scrollbar-pan-v305', { detail: { ...currentOffset, source, build: BUILD } }));
  scheduleCanvasState(`${source}:${direction}`);
  lastAction = { type: 'move', direction, offset: { ...currentOffset }, source, at: Date.now() };
}

function makePadButton(direction, className, markup, title) {
  const item = document.createElement('span');
  item.className = `gz452-pad ${className}`;
  ...
  item.addEventListener('pointerdown', (event) => {
    ...
    suppressClick = true;
    moveWheel(direction, 'pointer');
    holdDelay = window.setTimeout(() => {
      holdInterval = window.setInterval(() => moveWheel(direction, 'hold'), 75);
    }, 340);
  });
  ['pointerup', 'pointercancel', 'pointerleave'].forEach((name) => item.addEventListener(name, stopHold));
  item.addEventListener('click', (event) => {
    event.preventDefault();
    event.stopPropagation();
    if (suppressClick) { suppressClick = false; return; }
    moveWheel(direction, 'click');
  });
  item.addEventListener('keydown', (event) => {
    if (event.key !== 'Enter' && event.key !== ' ') return;
    event.preventDefault();
    moveWheel(direction, 'keyboard');
  });
  return item;
}
...
function createControl() {
  ...
  root.id = CONTROL_ID;
  root.dataset.gannzillaProtectedControlV452 = 'true';
  ...
  makeClickable(eye, (event) => {
    event.preventDefault();
    event.stopPropagation();
    setVisibility(!wheelVisible, 'eye');
  });
  ...
  makeClickable(move, (event) => {
    event.preventDefault();
    event.stopPropagation();
    moveWrap.dataset.open = moveWrap.dataset.open === 'true' ? 'false' : 'true';
  });

  const pad = document.createElement('div');
  pad.id = PAD_ID;
  pad.append(
    makePadButton('up', 'gz452-up', arrowMarkup('up'), ar ? 'تحريك للأعلى' : 'Move up'),
    makePadButton('left', 'gz452-left', arrowMarkup('left'), ar ? 'تحريك لليسار' : 'Move left'),
    makePadButton('center', 'gz452-center', centerMarkup(), ar ? 'السنتر والاتزان' : 'Center wheel'),
    makePadButton('right', 'gz452-right', arrowMarkup('right'), ar ? 'تحريك لليمين' : 'Move right'),
    makePadButton('down', 'gz452-down', arrowMarkup('down'), ar ? 'تحريك للأسفل' : 'Move down'),
  );
  ...
  makeClickable(zoomOut, (event) => {
    ...
    if (currentZoom > min) setZoom(currentZoom - step, 'zoom-out');
  });
  ...
  makeClickable(zoomIn, (event) => {
    ...
    if (currentZoom < max) setZoom(currentZoom + step, 'zoom-in');
  });
...
function install() {
  ...
  const refresh = () => {
    ...
    frame = window.requestAnimationFrame(() => {
      ...
      clearHiddenState(current);
      positionControl(current);
      updateEye();
      updateZoomControls();
      updateFullscreen();
      scheduleCanvasState('refresh');
    });
  };
  ...
  window.GANNZILLA_UNIFIED_WHEEL_TOOLS_V452 = true;
  window.__auditGannzillaUnifiedWheelToolsV452 = () => {
    const node = document.getElementById(CONTROL_ID);
    const rect = node?.getBoundingClientRect();
    const canvas = findWheelCanvas();
    return {
      ok: Boolean(node && rect?.width > 0 && rect?.height > 0 && canvas),
      build: BUILD,
      visibleControlCount: node?.children?.length || 0,
      protectedFromButtonCleaner: !node?.querySelector('button,[role="button"]'),
      protectedFromToolbarDedupe: !node?.hasAttribute('data-gannzilla-toolbar'),
      currentZoom,
      wheelVisible,
  ...
  window[STATE_KEY] = { observer, onDocumentClick, onKeyDown, onPanSync };
}
