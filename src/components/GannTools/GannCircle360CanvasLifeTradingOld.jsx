...
// CSS للحركات الانتقالية للبوكسات المنفصلة
const customStyles = `
  @keyframes slideDown {
    from {
      opacity: 0;
 ...
 * =================================
 * 
 * مكون متطور يجمع بين تقنيات جان التقليدية والأسواق المالية الحديثة
 * يوفر تحليلاً شاملاً ومتكاملاً للبيانات المالية مع دوائر جان
 */
...
const GannCircle360Canvas = ({
  // المعاملات المرسلة من الصفحة الرئيسية
  selectedMarkets: propSelectedMarkets = [],
  marketDataMode: propMarketDataMode = 'realtime',
  priceBasedColoring: propPriceBasedColoring = false,
  showMarketOverlay: propShowMarketOverlay = false,
  autoUpdateAnalysis: propAutoUpdateAnalysis = true,
  showTechnicalAnalysis: propShowTechnicalAnalysis = false,
  showMarketSettings: propShowMarketSettings = false,
  gannCircleSettings: propGannCircleSettings = null,
  // دوال التحكم
  onMarketsChange = () => {},
  onSettingsChange = () => {},
  // معاملات أخرى
  ...
  // الحصول على بيانات الأسواق المالية
  ...
  // States للأسواق المالية - استخدام المعاملات المرسلة أو القيم الافتراضية
  const [showMarketDataPanel, setShowMarketDataPanel] = useState(false);
  const [showTechnicalAnalysis, setShowTechnicalAnalysis] = useState(propShowTechnicalAnalysis);
  const [showMarketSettings, setShowMarketSettings] = useState(propShowMarketSettings);
  const [selectedMarkets, setSelectedMarkets] = useState(propSelectedMarkets);
  const [marketDataMode, setMarketDataMode] = useState(propMarketDataMode);
  const [priceBasedColoring, setPriceBasedColoring] = useState(propPriceBasedColoring);
  const [showMarketOverlay, setShowMarketOverlay] = useState(propShowMarketOverlay);
  const [autoUpdateAnalysis, setAutoUpdateAnalysis] = useState(propAutoUpdateAnalysis);
  const [showGlobalMarkets, setShowGlobalMarkets] = useState(false); // زر الأسواق العالمية الجديد
  
  // إعدادات إضافية للبوكسات المنفصلة
  const [activeMarketCategory, setActiveMarketCategory] = useState('crypto');
  const [startAngle, setStartAngle] = useState(0);
  const [endAngle, setEndAngle] = useState(360);
  const [minPrice, setMinPrice] = useState(0);
  const [maxPrice, setMaxPrice] = useState(100);
  const [updateInterval, setUpdateInterval] = useState(5); // فترة التحديث بالثواني
  
  // بيانات فئات الأسواق المالية
  const [marketCategories] = useState({
    crypto: [
      { symbol: 'BTC/USD', name: 'بيتكوين', price: '45,250.00', change: 2.5 },
      { symbol: 'ETH/USD', name: 'إيثريوم', price: '3,120.00', change: -1.2 },
      { symbol: 'ADA/USD', name: 'كاردانو', price: '0.48', change: 1.8 },
    ],
    forex: [
      { symbol: 'EUR/USD', name: 'يورو/دولار', price: '1.0850', change: 0.3 },
      { symbol: 'GBP/USD', name: 'جنيه/دولار', price: '1.2650', change: -0.5 },
      { symbol: 'USD/SAR', name: 'دولار/ريال', price: '3.75', change: 0.1 },
    ],
    stocks_us: [
      { symbol: 'AAPL', name: 'أبل', price: '175.50', change: 1.2 },
      { symbol: 'GOOGL', name: 'جوجل', price: '2,750.00', change: -0.8 },
      { symbol: 'MSFT', name: 'مايكروسوفت', price: '338.25', change: 0.9 },
    ],
    stocks_saudi: [
      { symbol: '2222', name: 'أرامكو', price: '32.50', change: 0.6 },
      { symbol: '1180', name: 'الراجحي', price: '78.20', change: -0.3 },
      { symbol: '2010', name: 'سابك', price: '95.40', change: 1.1 },
    ],
    commodities: [
      { symbol: 'GOLD', name: 'الذهب', price: '1,925.50', change: 0.8 },
      { symbol: 'SILVER', name: 'الفضة', price: '24.75', change: -1.1 },
      { symbol: 'OIL', name: 'النفط الخام', price: '82.30', change: 2.1 },
    ],
    indices: [
      { symbol: 'SPX', name: 'S&P 500', price: '4,185.50', change: 0.4 },
      { symbol: 'TASI', name: 'تاسي', price: '11,850.25', change: 0.7 },
      { symbol: 'DXY', name: 'مؤشر الدولار', price: '103.45', change: -0.2 },
    ],
  });
  ...
  // تحديث الحالات عند تغيير المعاملات المرسلة
  useEffect(() => {
    setSelectedMarkets(propSelectedMarkets);
  }, [propSelectedMarkets]);

  useEffect(() => {
    setMarketDataMode(propMarketDataMode);
  }, [propMarketDataMode]);

  useEffect(() => {
    setPriceBasedColoring(propPriceBasedColoring);
  }, [propPriceBasedColoring]);

  useEffect(() => {
    setShowMarketOverlay(propShowMarketOverlay);
  }, [propShowMarketOverlay]);

  useEffect(() => {
    setAutoUpdateAnalysis(propAutoUpdateAnalysis);
  }, [propAutoUpdateAnalysis]);

  useEffect(() => {
    setShowTechnicalAnalysis(propShowTechnicalAnalysis);
  }, [propShowTechnicalAnalysis]);

  useEffect(() => {
    setShowMarketSettings(propShowMarketSettings);
  }, [propShowMarketSettings]);

  useEffect(() => {
    if (propGannCircleSettings) {
      setSettings(propGannCircleSettings);
    }
  }, [propGannCircleSettings]);

  // إرسال التحديثات إلى الصفحة الرئيسية
  useEffect(() => {
    onMarketsChange(selectedMarkets);
  ...
  const findMarketInSector = (sector, angle) => {
    if (!selectedMarkets.length || !marketData) return null;
    ...
    for (const marketKey of selectedMarkets) {
      ...
      if (Math.abs(marketSector - sector) <= 1) {
        return {
          ...market,
          marketKey,
          movement: market.movement || ['up', 'down', 'neutral', 'reversal'][Math.floor(Math.random() * 4)]
        };
      }
    }
  ...
  // دالة للحصول على السعر الحالي للقطاع
  const getPriceForSector = (sector) => {
    const market = findMarketInSector(sector, 0);
    if (market && market.price) {
      return parseFloat(market.price).toFixed(2);
    }
    return null;
  };

  // دالة للتحقق من وجود أسعار في الحلقة الحالية
  const shouldShowPricesInLevel = (level) => {
    // إظهار الأسعار في الحلقات الأولى فقط عند وجود أسواق محددة
    return selectedMarkets.length > 0 && level <= 2;
  };
  ...
  if (shape === "star12") {
    ...
    if (settings.divisions > 0 && settings.levels > 0) {
      for (let level = 0; level < totalLevels; level++) {
        ...
        // رسم الأرقام والأسعار في جميع الحلقات (بدءاً من الحلقة الأولى)
        // عرض الأسعار مع الأرقام منذ البداية
        if (level >= 0) {
          ...
          // البحث عن السعر للقطاع الحالي
          const currentSector = (index % 36) + 1;
          const marketPrice = shouldShowPricesInLevel(level) ? getPriceForSector(currentSector) : null;
          
          // تنسيق الرقم بحد أقصى رقمين بعد الفاصلة العشرية
          const formattedValue = parseFloat(value.toFixed(2)).toString();
          
          // تحديد النص المراد عرضه - إما الرقم فقط أو الرقم مع السعر
          let displayText = formattedValue;
          if (marketPrice && selectedMarkets.length > 0) {
            displayText = `${formattedValue}\n$${marketPrice}`;
          }
          
          const valueString = displayText;
          
          // حساب حجم الخط بناءً على عرض الخلية وطول النص
          const maxFontSize = Math.min(cellWidth * 0.85, 24); // حد أقصى أكبر 24px
          const minFontSize = marketPrice ? 8 : 10; // حد أدنى أصغر عند وجود سعر
          
          // حساب حجم خط ديناميكي بناءً على طول النص
          let fontSize = Math.max(
            Math.min(maxFontSize, (cellWidth * 0.75) / Math.max(formattedValue.length, marketPrice ? marketPrice.length + 1 : 0) * 2.4), 
            minFontSize
          );
          
          // للنصوص التي تحتوي على سعر، نقلل الحجم قليلاً
          if (marketPrice) {
            fontSize = Math.max(fontSize * 0.8, minFontSize);
          }
          ...
          // رسم النص الأساسي
          ctx.strokeText(formattedValue, x, marketPrice ? y - fontSize * 0.4 : y);
          ctx.fillText(formattedValue, x, marketPrice ? y - fontSize * 0.4 : y);
          
          // رسم السعر إذا كان متوفراً (بتصميم محسن)
          if (marketPrice) {
            ctx.font = `bold ${fontSize * 0.7}px Arial`;
            
            // لون مميز للسعر حسب نوع الحركة
            const market = findMarketInSector(currentSector, 0);
            let priceColor = "#4CAF50"; // أخضر افتراضي
            if (market && market.movement === 'down') priceColor = "#f44336"; // أحمر للهبوط
            if (market && market.movement === 'up') priceColor = "#4CAF50"; // أخضر للصعود
            if (market && market.movement === 'neutral') priceColor = "#FF9800"; // برتقالي للثبات
            
            ctx.fillStyle = priceColor;
            ctx.strokeStyle = "rgba(0, 0, 0, 0.8)";
            ctx.lineWidth = 1;
            
            // إضافة خلفية للسعر للوضوح
            const priceText = `$${marketPrice}`;
            const textMetrics = ctx.measureText(priceText);
            const textWidth = textMetrics.width;
            const textHeight = fontSize * 0.7;
            
            // رسم خلفية نصف شفافة للسعر
            ctx.fillStyle = "rgba(0, 0, 0, 0.7)";
            ctx.fillRect(x - textWidth/2 - 2, y + fontSize * 0.3 - textHeight/2, textWidth + 4, textHeight);
            
            // رسم النص
            ctx.fillStyle = priceColor;
            ctx.strokeText(priceText, x, y + fontSize * 0.6);
            ctx.fillText(priceText, x, y + fontSize * 0.6);
          }
        ...
        <div style={{
          ...
          {showSettings && (
            <>
              ...
              {/* بوكس منفصل لعرض البيانات المباشرة */}
              {showMarketOverlay && (
                <div className="market-data-box" style={{
                  position: 'absolute',
                  bottom: '20px',
                  left: '50%',
                  ...
                  <h3 style={{
                    ...
                    🔗 البيانات المباشرة النشطة
                  </h3>
                  
                  {/* معلومات السوق الحالي */}
                  <div style={{
                    marginBottom: '20px',
                    padding: '15px',
                    ...
                    <div style={{
                      ...
                      <span style={{ 
                        fontWeight: 'bold', 
                        color: selectedMarkets && selectedMarkets.length > 0 && selectedMarkets[0].change >= 0 
                          ? '#4CAF50' : '#f44336' 
                      }}>
                        {selectedMarkets && selectedMarkets.length > 0 
                          ? `${selectedMarkets[0].change >= 0 ? '+' : ''}${selectedMarkets[0].change}%`
                          : '0.00%'}
                      </span>
                    </div>
                  </div>
                  
                  {/* معلومات موقع السعر على الدائرة */}
                  <div style={{
                    marginBottom: '20px',
                    padding: '15px',
                    backgroundColor: 'rgba(255, 255, 255, 0.1)',
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>الزاوية</div>
                      ...
                      <div style={{
                        ...
                        <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>المستوى</div>
                        <div style={{ color: '#FFD700' }}>
                          {selectedMarkets && selectedMarkets.length > 0 
                      ...
                      <div style={{
                        ...
                        <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>القطاع</div>
                        <div style={{ color: '#FFD700' }}>
                          {selectedMarkets && selectedMarkets.length > 0 
                            ? `Q${Math.round(Math.random() * 4) + 1}`
                            : 'Q1'}
                        </div>
                      </div>
                      <div style={{
                        ...
                        <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>الاتجاه</div>
                        <div style={{ 
                          color: selectedMarkets && selectedMarkets.length > 0 && selectedMarkets[0].change >= 0 
                            ? '#4CAF50' : '#f44336' 
                        }}>
                          {selectedMarkets && selectedMarkets.length > 0 
                            ? (selectedMarkets[0].change >= 0 ? '⬆️ صعود' : '⬇️ هبوط')
                            : '➡️ مستقر'}
                        </div>
                      </div>
                    </div>
                  ...
                  <div style={{
                    ...
                    <span style={{ 
                      color: autoUpdateAnalysis ? '#4CAF50' : '#f44336',
                      fontWeight: 'bold' 
                    }}>
                      {autoUpdateAnalysis ? '✅ مفعل' : '❌ معطل'}
                    </span>
                  </div>
                </div>
              )}
              ...
              {/* بوكس منفصل للتلوين الذكي */}
              {priceBasedColoring && (
                <div className="market-control-box" style={{
                  position: 'absolute',
                  bottom: '20px',
                  left: '20px',
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <span style={{ color: '#4CAF50', fontWeight: 'bold' }}>✅ نشط</span>
                    </div>
                  </div>
                  ...
                  <button 
                    onClick={() => {
                      setPriceBasedColoring(false);
                      console.log('تم إيقاف التلوين الذكي');
                    }}
                    style={{
                      width: '100%',
                      padding: '12px',
              ...
              {showTechnicalAnalysis && (
                <div className="market-control-box" style={{
                  ...
                  <div style={{
                    ...
                    <button 
                      onClick={() => {
                        console.log('تحليل جان المتقدم');
                        // يمكن إضافة منطق تفعيل تحليل جان هنا
                      }}
                      style={{
                        padding: '12px 16px',
                    ...
                    <button 
                      onClick={() => {
                        console.log('الأنماط الكلاسيكية');
                      }}
                      style={{
                        padding: '12px 16px',
                        backgroundColor: 'rgba(255, 255, 255, 0.2)',
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: '12px', opacity: 0.8 }}>
                      الأدوات جاهزة للاستخدام - اختر الأداة المطلوبة
                    </div>
                  </div>
                </div>
              )}

              {/* بوكس منفصل للأسواق المالية العالمية */}
              {showGlobalMarkets && (
                <div className="market-control-box" style={{
                  position: 'absolute',
                  top: '150px',
                  ...
                  <div style={{
                    ...
                    onClick={() => console.log('السوق الأمريكي')}
                    onMouseEnter={(e) => {
                    ...
                    onClick={() => console.log('السوق الأوروبي')}
                    onMouseEnter={(e) => {
                      e.target.style.transform = 'scale(1.05)';
                    ...
                    onMouseLeave={(e) => {
                      e.target.style.transform = 'scale(1)';
                      e.target.style.backgroundColor = 'rgba(156, 39, 176, 0.8)';
                    }}
                    >
                      🇪🇺 LSE & DAX
                    </div>
                    ...
                    onClick={() => console.log('السوق الآسيوي')}
                    onMouseEnter={(e) => {
                      e.target.style.transform = 'scale(1.05)';
                    ...
                    onMouseLeave={(e) => {
                      e.target.style.transform = 'scale(1)';
                      e.target.style.backgroundColor = 'rgba(255, 152, 0, 0.8)';
                    }}
                    >
                      🇯🇵 Nikkei & ASX
                    </div>
                    ...
                    onClick={() => console.log('الأسواق الناشئة')}
                    onMouseEnter={(e) => {
                      e.target.style.transform = 'scale(1.05)';
                      e.target.style.backgroundColor = '#795548';
                    }}
                    onMouseLeave={(e) => {
                      e.target.style.transform = 'scale(1)';
                      e.target.style.backgroundColor = 'rgba(121, 85, 72, 0.8)';
                    }}
                    >
                      🌏 الأسواق الناشئة
                    </div>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <span>الأسواق المفتوحة:</span>
                      <span style={{ color: '#4CAF50', fontWeight: 'bold' }}>15</span>
                    </div>
                    <div style={{
                      display: 'flex',
                      justifyContent: 'space-between',
                      ...
                      <span>الأسواق المغلقة:</span>
                      <span style={{ color: '#f44336', fontWeight: 'bold' }}>8</span>
                    </div>
                    <div style={{
                      display: 'flex',
                      justifyContent: 'space-between',
                      ...
                      <span style={{ color: '#FFD700', fontWeight: 'bold' }}>⏰ نشط</span>
                    </div>
                  </div>
                  
                  {/* زر تحديث البيانات العالمية */}
                  <button 
                    onClick={() => {
                      console.log('تحديث البيانات العالمية');
                      // يمكن إضافة منطق تحديث البيانات العالمية هنا
                    }}
                    style={{
                      width: '100%',
                      padding: '12px',
              ...
              {/* بوكس منفصل للأسواق المالية المحددة - بنفس ستايل التلوين الذكي */}
              {showMarketDataPanel && (
                <div className="market-data-box" style={{
                  position: 'absolute',
                  top: '150px',
                  ...
                  <div style={{
                    ...
                    <button 
                      onClick={() => {
                        setActiveMarketCategory('crypto');
                        console.log('تم اختيار العملات الرقمية');
                      }}
                      style={{
                        padding: '12px',
                        backgroundColor: activeMarketCategory === 'crypto' ? 'rgba(33, 150, 243, 1)' : 'rgba(33, 150, 243, 0.8)',
                        color: 'white',
                        borderRadius: '8px',
                        textAlign: 'center',
                        fontSize: '14px',
                        fontWeight: 'bold',
                        border: activeMarketCategory === 'crypto' ? '3px solid #FFD700' : '2px solid #2196F3',
                        cursor: 'pointer',
                        transition: 'all 0.3s ease',
                      }}
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'scale(1.05)';
                        e.target.style.boxShadow = '0 4px 15px rgba(33, 150, 243, 0.6)';
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'scale(1)';
                    ...
                    <button 
                      onClick={() => {
                        setActiveMarketCategory('forex');
                        console.log('تم اختيار الفوركس');
                      }}
                      style={{
                        padding: '12px',
                        backgroundColor: activeMarketCategory === 'forex' ? 'rgba(156, 39, 176, 1)' : 'rgba(156, 39, 176, 0.8)',
                        color: 'white',
                        borderRadius: '8px',
                        textAlign: 'center',
                        fontSize: '14px',
                        fontWeight: 'bold',
                        border: activeMarketCategory === 'forex' ? '3px solid #FFD700' : '2px solid #9C27B0',
                        cursor: 'pointer',
                        transition: 'all 0.3s ease',
                      }}
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'scale(1.05)';
                        e.target.style.boxShadow = '0 4px 15px rgba(156, 39, 176, 0.6)';
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'scale(1)';
                        e.target.style.boxShadow = 'none';
                    ...
                    <button 
                      onClick={() => {
                        setActiveMarketCategory('stocks_us');
                        console.log('تم اختيار السوق الأمريكي');
                      }}
                      style={{
                        padding: '12px',
                        backgroundColor: activeMarketCategory === 'stocks_us' ? 'rgba(255, 152, 0, 1)' : 'rgba(255, 152, 0, 0.8)',
                        color: 'white',
                        borderRadius: '8px',
                        textAlign: 'center',
                        fontSize: '14px',
                        fontWeight: 'bold',
                        border: activeMarketCategory === 'stocks_us' ? '3px solid #FFD700' : '2px solid #FF9800',
                        cursor: 'pointer',
                        transition: 'all 0.3s ease',
                      }}
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'scale(1.05)';
                        e.target.style.boxShadow = '0 4px 15px rgba(255, 152, 0, 0.6)';
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'scale(1)';
                        e.target.style.boxShadow = 'none';
                    ...
                    <button 
                      onClick={() => {
                        setActiveMarketCategory('stocks_saudi');
                        console.log('تم اختيار السوق السعودي');
                      }}
                      style={{
                        padding: '12px',
                        backgroundColor: activeMarketCategory === 'stocks_saudi' ? 'rgba(121, 85, 72, 1)' : 'rgba(121, 85, 72, 0.8)',
                        color: 'white',
                        borderRadius: '8px',
                        textAlign: 'center',
                        fontSize: '14px',
                        fontWeight: 'bold',
                        border: activeMarketCategory === 'stocks_saudi' ? '3px solid #FFD700' : '2px solid #795548',
                        cursor: 'pointer',
                        transition: 'all 0.3s ease',
                      }}
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'scale(1.05)';
                        e.target.style.boxShadow = '0 4px 15px rgba(121, 85, 72, 0.6)';
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'scale(1)';
                        e.target.style.boxShadow = 'none';
                    ...
                    <button 
                      onClick={() => {
                        setActiveMarketCategory('commodities');
                        console.log('تم اختيار السلع');
                      }}
                      style={{
                        padding: '12px',
                        backgroundColor: activeMarketCategory === 'commodities' ? 'rgba(255, 193, 7, 1)' : 'rgba(255, 193, 7, 0.8)',
                        color: '#333',
                        borderRadius: '8px',
                        textAlign: 'center',
                        fontSize: '14px',
                        fontWeight: 'bold',
                        border: activeMarketCategory === 'commodities' ? '3px solid #FFD700' : '2px solid #FFC107',
                        cursor: 'pointer',
                        transition: 'all 0.3s ease',
                      }}
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'scale(1.05)';
                        e.target.style.boxShadow = '0 4px 15px rgba(255, 193, 7, 0.6)';
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'scale(1)';
                        e.target.style.boxShadow = 'none';
                    ...
                    <button 
                      onClick={() => {
                        setActiveMarketCategory('indices');
                        console.log('تم اختيار المؤشرات');
                      }}
                      style={{
                        padding: '12px',
                        backgroundColor: activeMarketCategory === 'indices' ? 'rgba(96, 125, 139, 1)' : 'rgba(96, 125, 139, 0.8)',
                        color: 'white',
                        borderRadius: '8px',
                        textAlign: 'center',
                        fontSize: '14px',
                        fontWeight: 'bold',
                        border: activeMarketCategory === 'indices' ? '3px solid #FFD700' : '2px solid #607D8B',
                        cursor: 'pointer',
                        transition: 'all 0.3s ease',
                      }}
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'scale(1.05)';
                        e.target.style.boxShadow = '0 4px 15px rgba(96, 125, 139, 0.6)';
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'scale(1)';
                        e.target.style.boxShadow = 'none';
                  ...
                  <div style={{
                    ...
                    <h4 style={{ 
                      ...
                      {activeMarketCategory === 'crypto' && '💰 العملات الرقمية'}
                      {activeMarketCategory === 'forex' && '💱 أزواج العملات'}
                      {activeMarketCategory === 'stocks_us' && '🇺🇸 الأسهم الأمريكية'}
                      {activeMarketCategory === 'stocks_saudi' && '🇸🇦 الأسهم السعودية'}
                      {activeMarketCategory === 'commodities' && '🥇 السلع والمعادن'}
                      {activeMarketCategory === 'indices' && '📊 المؤشرات العالمية'}
                    </h4>
                    
                    {marketCategories[activeMarketCategory] && marketCategories[activeMarketCategory].length > 0 ? (
                      marketCategories[activeMarketCategory].map((market, index) => (
                        <div key={index} style={{
                          display: 'flex',
                          justifyContent: 'space-between',
                          alignItems: 'center',
                        ...
                        onClick={() => {
                          console.log(`تم اختيار السوق: ${market.name} (${market.symbol})`);
                          // يمكن إضافة منطق لتحديد السوق وعرض تفاصيله
                        }}
                        onMouseEnter={(e) => {
                        ...
                        >
                          ...
                          <div style={{ textAlign: 'right', color: 'white' }}>
                            ...
                            <div style={{ 
                              fontSize: '12px', 
                              color: market.change >= 0 ? '#00FF88' : '#FF4444',
                              fontWeight: 'bold'
                            }}>
                      ...
                      <div style={{
                        ...
                        لا توجد أسواق متاحة في هذه الفئة
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <span>الأسواق المتاحة:</span>
                      <span style={{ color: '#4CAF50', fontWeight: 'bold' }}>
                        {marketCategories ? Object.keys(marketCategories).length : 6}
                      </span>
                    </div>
                    <div style={{
                      display: 'flex',
                      ...
                      <span>السوق النشط:</span>
                      <span style={{ color: '#FFD700', fontWeight: 'bold' }}>
                        {activeMarketCategory || 'crypto'}
                      </span>
                    </div>
                    <div style={{
                      display: 'flex',
                      ...
                      <span>حالة النظام:</span>
                      <span style={{ color: '#4CAF50', fontWeight: 'bold' }}>✅ متصل</span>
                    </div>
                  </div>
                  ...
                  <button 
                    onClick={() => {
                      console.log('تحديث بيانات الأسواق');
              ...
              {/* بوكس منفصل للأسواق المالية - ثابت فوق الدائرة */}
              <div className="market-control-box" style={{
                position: 'fixed',
                top: '10px',
                left: '50%',
                ...
                <div style={{
                  ...
                  <button 
                    onClick={() => {
                      console.log('الأسواق المالية تم النقر');
                      setShowMarketDataPanel(!showMarketDataPanel);
                    }}
                    style={{
                      padding: '8px 10px',
                      backgroundColor: showMarketDataPanel ? '#4CAF50' : 'rgba(76, 175, 80, 0.2)',
                      color: showMarketDataPanel ? 'white' : '#4CAF50',
                      border: `1px solid ${showMarketDataPanel ? '#4CAF50' : '#4CAF50'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '11px',
                      fontWeight: 'bold',
                  ...
                  <button 
                    ...
                    style={{
                      padding: '8px 10px',
                      backgroundColor: showTechnicalAnalysis ? '#2196F3' : 'rgba(33, 150, 243, 0.2)',
                      color: showTechnicalAnalysis ? 'white' : '#2196F3',
                      border: `1px solid ${showTechnicalAnalysis ? '#2196F3' : '#2196F3'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '11px',
                      fontWeight: 'bold',
                  ...
                  <button 
                    ...
                    style={{
                      padding: '8px 10px',
                      backgroundColor: priceBasedColoring ? '#FF9800' : 'rgba(255, 152, 0, 0.2)',
                      color: priceBasedColoring ? 'white' : '#FF9800',
                      border: `1px solid ${priceBasedColoring ? '#FF9800' : '#FF9800'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '11px',
                      fontWeight: 'bold',
                  ...
                  <button 
                    ...
                    style={{
                      padding: '8px 10px',
                      backgroundColor: showMarketOverlay ? '#9C27B0' : 'rgba(156, 39, 176, 0.2)',
                      color: showMarketOverlay ? 'white' : '#9C27B0',
                      border: `1px solid ${showMarketOverlay ? '#9C27B0' : '#9C27B0'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '11px',
                      fontWeight: 'bold',
                  ...
                  <button 
                    ...
                    style={{
                      padding: '8px 10px',
                      backgroundColor: showMarketSettings ? '#795548' : 'rgba(121, 85, 72, 0.2)',
                      color: showMarketSettings ? 'white' : '#795548',
                      border: `1px solid ${showMarketSettings ? '#795548' : '#795548'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '11px',
                      fontWeight: 'bold',
                    ...
                    onMouseEnter={(e) => {
                      e.target.style.transform = 'translateY(-1px)';
                      e.target.style.boxShadow = '0 2px 8px rgba(121, 85, 72, 0.4)';
                    }}
                    onMouseLeave={(e) => {
                      e.target.style.transform = 'translateY(0)';
                      e.target.style.boxShadow = 'none';
                  ...
                  <button 
                    onClick={() => {
                      console.log('التحديث الآلي تم النقر');
                      setAutoUpdateAnalysis(!autoUpdateAnalysis);
                    }}
                    style={{
                      padding: '8px 10px',
                      backgroundColor: autoUpdateAnalysis ? '#607D8B' : 'rgba(96, 125, 139, 0.2)',
                      color: autoUpdateAnalysis ? 'white' : '#607D8B',
                      border: `1px solid ${autoUpdateAnalysis ? '#607D8B' : '#607D8B'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '11px',
                      fontWeight: 'bold',
                    ...
                    onMouseEnter={(e) => {
                      e.target.style.transform = 'translateY(-1px)';
                      e.target.style.boxShadow = '0 2px 8px rgba(96, 125, 139, 0.4)';
                    }}
                    onMouseLeave={(e) => {
                      e.target.style.transform = 'translateY(0)';
                      e.target.style.boxShadow = 'none';
                  ...
                  <button 
                    onClick={() => {
                      console.log('الأسواق العالمية تم النقر');
                      setShowGlobalMarkets(!showGlobalMarkets);
                    }}
                    style={{
                      padding: '8px 10px',
                      backgroundColor: showGlobalMarkets ? '#4CAF50' : 'rgba(76, 175, 80, 0.2)',
                      color: showGlobalMarkets ? 'white' : '#4CAF50',
                      border: `1px solid ${showGlobalMarkets ? '#4CAF50' : '#4CAF50'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '11px',
                      fontWeight: 'bold',
...
{showRepeatedPattern && (
  <>
    ...
    >
      ...
      {/* ⚙️ إعدادات دائرة جان المتقدمة - يظهر فقط عند تفعيل إعدادات السوق */}
      {showMarketSettings && (
        <div className="market-box" style={{
          ...
          <div style={{
            ...
            right: 0,
            height: '4px',
            background: 'linear-gradient(90deg, #607D8B, #78909C, #607D8B)',
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{
              ...
              <div>
                <label style={{ fontSize: '12px', color: '#B0BEC5', display: 'block', marginBottom: '5px' }}>
                  زاوية البداية
                </label>
                <input
                  type="number"
                  ...
                  onChange={(e) => setSettings(prev => ({ ...prev, startAngle: parseFloat(e.target.value) }))}
                  style={{
                    width: '100%',
                    padding: '8px',
                    backgroundColor: 'rgba(255, 255, 255, 0.1)',
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSettings(prev => ({ ...prev, endAngle: parseFloat(e.target.value) }))}
                  style={{
                    width: '100%',
                    padding: '8px',
                    backgroundColor: 'rgba(255, 255, 255, 0.1)',
            ...
            <div style={{
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSettings(prev => ({ ...prev, minPrice: parseFloat(e.target.value) }))}
                  style={{
                    width: '100%',
                    padding: '8px',
                    backgroundColor: 'rgba(255, 255, 255, 0.1)',
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSettings(prev => ({ ...prev, maxPrice: parseFloat(e.target.value) }))}
                  style={{
                    width: '100%',
                    padding: '8px',
                    backgroundColor: 'rgba(255, 255, 255, 0.1)',
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div>
              ...
              >
                <option value="1">كل ثانية (لحظي)</option>
            ...
            <div style={{
              ...
              <label style={{
                ...
                <input
                  ...
                  onChange={(e) => setPriceBasedColoring(e.target.checked)}
                  style={{
                    accentColor: '#607D8B',
                    transform: 'scale(1.2)'
                  }}
