const BUILD = 528;
const STATE_KEY = '__gannzillaSingleAnglePreviewBridgeV528';
...
function refreshVisibleComposite(source = 'refresh') {
  const angleAudit = window.__auditGannzillaSingleAngleOuterRingV527?.();
  ...
  previewAuthority.scheduleRender(`single-angle-composite-${source}-v528`, 0);
  refreshCount += 1;
  lastRefresh = {
    source,
    build: BUILD,
...
function install() {
  ...
  window.addEventListener('gannzilla:single-angle-outer-ring-v527', () => {
    schedule('angle-v527');
  }, false);
  ...
  window.GANNZILLA_SINGLE_ANGLE_PREVIEW_BRIDGE_V528 = true;
  window.__auditGannzillaSingleAnglePreviewBridgeV528 = () => {
    const angleAudit = window.__auditGannzillaSingleAngleOuterRingV527?.();
