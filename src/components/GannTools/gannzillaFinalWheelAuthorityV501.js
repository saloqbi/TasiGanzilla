const BUILD = 501;
const STATE_KEY = '__gannzillaFinalWheelAuthorityV501';
...
function draw(source = 'refresh') {
  ...
  canvas.dataset.gannzillaFinalWheelAuthorityV501 = 'true';
  ...
  window.dispatchEvent(new CustomEvent('gannzilla:final-wheel-authority-v501', {
    detail: { ...lastDraw, build: BUILD },
  }));
  return true;
}
...
function install() {
  ...
  window.GANNZILLA_FINAL_WHEEL_AUTHORITY_V501 = true;
  window.GANNZILLA_PURE_WHITE_CELLS_V501 = true;
  window.__auditGannzillaFinalWheelAuthorityV501 = () => {
    const canvas = findWheel();
    const cfg = settings();
