...
const BUILD = 219;
const MOVE_ID = 'gannzilla-wheel-pan-v219';
const FIT_ID = 'gannzilla-wheel-fit-v219';
const NATIVE_FIT_ATTR = 'data-gannzilla-native-fit-v219';
...
function looksLikeFitButton(button) {
  if (!button || button.id === MOVE_ID || button.id === FIT_ID) return false;
...
function fallbackFitToViewport() {
  ...
  window.setTimeout(centerViewport, 20);
  return true;
}
...
function getRects() {
  ...
  if (!nativeFit) {
    const fit = { left: Math.max(36, window.innerWidth - 164), top: 1, width: 26, height: 22 };
    return {
      move: { ...fit, left: fit.left - 28 },
      fit,
      nativeFit: null,
    };
  }

  nativeFit.setAttribute(NATIVE_FIT_ATTR, 'true');
  nativeFit.style.setProperty('margin-left', '30px', 'important');
  const fitRect = nativeFit.getBoundingClientRect();
  const fit = {
    left: Math.round(fitRect.left),
    top: Math.round(fitRect.top),
    width: Math.max(24, Math.round(fitRect.width)),
    height: Math.max(21, Math.round(fitRect.height)),
  };

  return {
    move: { ...fit, left: fit.left - 28, width: 26 },
    fit,
    nativeFit,
  };
}
...
export default function GannzillaWheelPanControlV180() {
  const [layout, setLayout] = React.useState(getRects);
  ...
  React.useEffect(() => {
    window.GANNZILLA_WHEEL_NAVIGATION_V219 = true;
    window.__auditGannzillaWheelNavigationV219 = () => ({
