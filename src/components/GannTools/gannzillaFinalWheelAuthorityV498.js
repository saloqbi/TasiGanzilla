const BUILD = 498;
const STATE_KEY = '__gannzillaFinalWheelAuthorityV498';
...
function drawNumber(ctx, value, x, y, dpr) {
  ...
  // Deliberately omit Canvas maxWidth so the browser never compresses the digits.
...
function draw(source = 'refresh') {
  ...
  // Preserve the accepted layout exactly:
  // ring 1 = 1..36, ring 2 = blank, startValue begins at ring 3.
  ...
  canvas.dataset.gannzillaFinalWheelAuthorityV498 = 'true';
  ...
  window.dispatchEvent(new CustomEvent('gannzilla:final-wheel-authority-v498', {
    detail: { ...lastDraw, build: BUILD },
  }));
  return true;
}
...
function install() {
  ...
  window.GANNZILLA_FINAL_WHEEL_AUTHORITY_V498 = true;
  window.GANNZILLA_REFERENCE_ARIAL_TYPOGRAPHY_V498 = true;
  window.__auditGannzillaFinalWheelAuthorityV498 = () => {
    const canvas = findWheel();
    const cfg = settings();
