...
function AngleReferenceIcon({ value }) {
  return (
    <svg width="80" height="80" viewBox="0 0 80 80" aria-hidden="true">
      ...
      <text x="40" y="48" textAnchor="middle" fontFamily="Segoe UI, Arial, sans-serif" fontSize="30" fontWeight="700" fill="#74787b">
        {value}°
      </text>
    </svg>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        margin: 0,
        border: round
          ? (active ? '3px solid #6f82b7' : '2px solid #bfc4c8')
          : (active ? '2px solid #8799bc' : '1px solid transparent'),
...
export default function GannzillaLeftReferencePaletteV129() {
  ...
  const choose = (value) => {
    ...
    window.dispatchEvent(new CustomEvent('gannzilla:left-reference-tool-v129', {
      detail: { tool: value },
    }));
  };
