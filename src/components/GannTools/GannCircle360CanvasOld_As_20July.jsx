...
useEffect(() => {
  ...
  function onMouseDown(e) {
    ...
    // **أولوية عالية جداً: التعامل مع المثلث أولاً عندما يكون محدد**
    if (selectedShape === "triangle") {
      console.log("🔺🎯 SUPER PRIORITY: Triangle is selected - checking triangle BEFORE everything else");
      ...
      console.log(`🔺 Triangle super priority check - radius: ${r}, center: (${center}, ${center})`);
      ...
      for (let idx = 0; idx < trianglePoints.length; idx++) {
        ...
        console.log(`🔺 SUPER PRIORITY Vertex ${idx} (angle: ${vertexAngle}°): position (${point.x.toFixed(1)}, ${point.y.toFixed(1)}), distance: ${d.toFixed(1)}`);
        ...
        if (d < detectionRadius) {
          console.log(`🚀 TRIANGLE SUPER PRIORITY: IMMEDIATE activation on vertex ${idx} at angle ${vertexAngle}° (distance: ${d.toFixed(1)})`);
          console.log(`🚀 Overriding ALL other detections - Triangle has absolute priority!`);
      ...
      for (let i = 0; i < trianglePoints.length; i++) {
        ...
        if (distanceToEdge < 30) { // منطقة أوسع للأضلاع
          console.log(`🚀 TRIANGLE SUPER PRIORITY: Edge detection ${i}-${(i + 1) % trianglePoints.length} (distance: ${distanceToEdge.toFixed(1)})`);
          setIsDraggingTriangle(true);
          setDragStartAngle(getAngleDeg(x, y));
          setInitialRotation(triangleRotation);
          return;
      ...
      for (let idx = 0; idx < trianglePoints.length; idx++) {
        ...
        if (distanceToRay < 35 && distanceFromCenter >= 50 && distanceFromCenter <= r + 100) {
          console.log(`🚀 TRIANGLE SUPER PRIORITY: Ray detection to vertex ${idx} (distance: ${distanceToRay.toFixed(1)})`);
          setIsDraggingTriangle(true);
          setDragStartAngle(getAngleDeg(x, y));
          setInitialRotation(triangleRotation);
          return;
    ...
    // **أولوية متوسطة: التعامل مع عجلة الزوايا (إذا كانت مفعلة) - بعد التأكد من عدم التداخل مع المثلث**
    if (showAngleWheel) {
      console.log("🧲 Secondary priority check for angle wheel...");
      
      // **فحص إضافي: التأكد من عدم التداخل مع رؤوس المثلث إذا كان محدد**
      if (selectedShape === "triangle") {
        console.log(⚠️ Triangle is selected - double checking for triangle vertices before angle wheel");
        const r = degreeRingRadius;
        const trianglePoints = customAngles.map((deg) => {
          const rad = ((deg + triangleRotation + settings.rotation - 90) * Math.PI) / 180;
          return {
        ...
        // فحص نهائي لرؤوس المثلث قبل عجلة الزوايا
        for (let idx = 0; idx < trianglePoints.length; idx++) {
          const point = trianglePoints[idx];
          const d = getDistance(x, y, point.x, point.y);
          const vertexAngle = customAngles[idx];
          
          if (d < 50) { // منطقة حماية واسعة جداً للمثلث
            console.log(`🛡️ TRIANGLE PROTECTION: Blocking angle wheel - too close to triangle vertex ${idx} at ${vertexAngle}° (distance: ${d.toFixed(1)})`);
            console.log(`🔺 Activating triangle instead of angle wheel`);
            setIsDraggingTriangle(true);
            setDragStartAngle(getAngleDeg(x, y));
            setInitialRotation(triangleRotation);
            return; // أولوية مطلقة للمثلث
          }
        }
      }
      
      console.log(`📐 angleWheelInnerRadius: ${angleWheelInnerRadius}, angleWheelOuterRadius: ${angleWheelOuterRadius}`);
      console.log(`📏 Distance from center: ${distanceFromCenter.toFixed(1)}`);
      
      // **الحل الجذري المحسن**: كشف النقر على عجلة الزوايا وخطوطها المتقاطعة
      
      // 1️⃣ أولاً: كشف النقر على الخطوط المتقاطعة داخل حلقة عجلة الزوايا
      let clickedOnAngleWheelLines = false;
      
      if (distanceFromCenter >= angleWheelInnerRadius && distanceFromCenter <= angleWheelOuterRadius) {
        console.log(`🎯 Clicked directly on angle wheel ring! Distance: ${distanceFromCenter.toFixed(1)}`);
        
        // كشف النقر على الخطوط المتقاطعة (خطوط التقسيم)
        const stepDegrees = angleStepRad;
        const totalCells = 360 / stepDegrees;
        
        for (let i = 0; i < totalCells; i++) {
          const cellAngle = angleWheelRotation + i * stepDegrees;
          const cellRad = (cellAngle * Math.PI) / 180;
          
          // حساب الخط الشعاعي من الحافة الداخلية إلى الخارجية
          const innerX = center + angleWheelInnerRadius * Math.cos(cellRad);
          const innerY = center + angleWheelInnerRadius * Math.sin(cellRad);
          const outerX = center + angleWheelOuterRadius * Math.cos(cellRad);
          const outerY = center + angleWheelOuterRadius * Math.sin(cellRad);
          
          const distanceToRadialLine = getDistanceToLine(x, y, innerX, innerY, outerX, outerY);
          
          if (distanceToRadialLine < 15) { // مسافة تساهل للخطوط الشعاعية
            console.log(`⚡ Clicked on radial division line ${i} in angle wheel!`);
            clickedOnAngleWheelLines = true;
            break;
          }
        }
        
        // كشف النقر على الخطوط الدائرية (الحافة الداخلية والخارجية)
        const distanceToInnerCircle = Math.abs(distanceFromCenter - angleWheelInnerRadius);
        const distanceToOuterCircle = Math.abs(distanceFromCenter - angleWheelOuterRadius);
        
        if (distanceToInnerCircle < 15 || distanceToOuterCircle < 15) {
          console.log(`🔵 Clicked on circular border of angle wheel!`);
          clickedOnAngleWheelLines = true;
        }
        
        // إذا كان النقر في أي مكان داخل حلقة عجلة الزوايا (حتى لو لم يكن على الخطوط)
        if (clickedOnAngleWheelLines || true) { // true يعني أي نقرة داخل الحلقة مقبولة
          console.log(`✅ ANGLE WHEEL RING: Activating rotation from angle wheel area`);
      ...
      // 2️⃣ ثانياً: كشف النقر على الخطوط الشعاعية الممتدة من المركز
      const stepDegrees = angleStepRad;
      const totalCells = 360 / stepDegrees;
      ...
      for (let i = 0; i < totalCells; i++) {
        ...
        const distanceToLine = getDistanceToLine(
          x, y,
          center, center,
          center + angleWheelOuterRadius * Math.cos(rayAngle),
          center + angleWheelOuterRadius * Math.sin(rayAngle)
        );
        
        if (distanceToLine < 25 && distanceFromCenter >= 50 && distanceFromCenter <= angleWheelOuterRadius + 100) {
          console.log(`✅ RAY LINE: Clicked on extended ray ${i} - activating rotation`);
          setIsDraggingAngleWheel(true);
          setDragStartAngle(getAngleDeg(x, y));
          setInitialRotation(angleWheelRotation);
          return; // خروج فوري
      ...
      // 3️⃣ ثالثاً: منطقة عامة واسعة (إذا لم تعمل الطرق السابقة)
      const minRange = 50;
      const maxRange = canvasSize / 2 - 50;
      
      if (distanceFromCenter >= minRange && distanceFromCenter <= maxRange) {
        console.log(`✅ UNIVERSAL ROTATION: Clicked at distance ${distanceFromCenter.toFixed(1)} - activating angle wheel drag`);
        console.log(`📌 Range: ${minRange}-${maxRange}, Click accepted anywhere in this range`);
        setIsDraggingAngleWheel(true);
        setDragStartAngle(getAngleDeg(x, y));
        setInitialRotation(angleWheelRotation);
        return; // خروج فوري - أولوية عالية
      } else {
        console.log(`❌ Click outside universal range. Allowed: ${minRange}-${maxRange}, Got: ${distanceFromCenter.toFixed(1)}`);
      }
    }
    
    // **التعامل مع الأشكال الأخرى (أولوية أقل)**
    ...
    // **منطق احتياطي لعجلة الزوايا - فقط إذا لم يكن المثلث محدد أو لم يتم تفعيله**
    if (showAngleWheel && !isDraggingAngleWheel && selectedShape !== "triangle") {
      console.log(`🔄 ANGLE WHEEL FALLBACK: Final attempt for angle wheel (triangle not selected). Distance: ${distanceFromCenter.toFixed(1)}`);
      
      // أي نقرة في أي مكان معقول تفعل عجلة الزوايا - فقط إذا لم يكن المثلث محدد
      if (distanceFromCenter >= 30 && distanceFromCenter <= (canvasSize / 2 - 30)) {
        console.log(`✅ ANGLE WHEEL FALLBACK SUCCESS: Activating angle wheel rotation (safe - no triangle selected)`);
        setIsDraggingAngleWheel(true);
        setDragStartAngle(getAngleDeg(x, y));
        setInitialRotation(angleWheelRotation);
      }
  ...
  function onMouseMove(e) {
    ...
    if (showAngleWheel) {
      ...
      if (isDraggingAngleWheel) {
        ...
        console.log(`Current angle: ${currentAngle.toFixed(1)}°, delta: ${delta.toFixed(1)}°, new rotation: ${newRotation.toFixed(1)}°`);
  ...
  function drawWeekDaysRing(ctx, center) {
    ...
    // رسم الخلايا
    for (let i = 0; i < 36; i++) {
      const centerAngleDeg = -90 + 10 + i * angleStep; // البداية من 10 درجة
    ...
    for (let i = 0; i < 36; i++) {
      const textAngleDeg = -90 + 10 + i * angleStep;
      const textAngleRad = (textAngleDeg * Math.PI) / 180;
      const textRadius = (radiusInner + radiusOuter) / 2;
      ...
      // نتبع النمط: أحد(1)، اثنين(2)، ثلاثاء(3)، أربعاء(4)، خميس(5)، جمعة(6)، أحد(7)، اثنين(8)، ثلاثاء(9)، أربعاء(1)...
      const dayIndex = (i % 6); // 6 أيام فقط (من الأحد إلى الجمعة)
      
      const dayName = weekDays[dayIndex];
      const angleValue = (i + 1) * 10; // الزاوية من 10 إلى 360
