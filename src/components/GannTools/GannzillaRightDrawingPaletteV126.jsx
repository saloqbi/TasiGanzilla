...
const PALETTE_ID = 'gannzilla-right-drawing-palette-v126';
const COLORS = {
  3: '#b8c8b9',
  4: '#d7c0c7',
  5: '#a8bac7',
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: 0,
        margin: 0,
        border: active ? '1px solid #c9d7df' : '1px solid transparent',
        borderRadius: 4,
...
function findLegacyRightPalettes() {
  return Array.from(document.querySelectorAll('div')).filter((element) => {
    if (element.id === PALETTE_ID || element.dataset.gannzillaRightPaletteV126 === 'true') return false;
...
export default function GannzillaRightDrawingPaletteV126() {
  ...
  const [top, setTop] = React.useState(168);

  React.useEffect(() => {
    const sync = () => {
      findLegacyRightPalettes().forEach((element) => {
        element.dataset.gannzillaLegacyRightPaletteHiddenV126 = 'true';
      ...
      const paletteHeight = 11 * 38 + 10 * 3 + 12;
    ...
    window.GANNZILLA_RIGHT_DRAWING_PALETTE_V126 = true;
    window.__auditGannzillaRightDrawingPaletteV126 = () => ({
  ...
  const choose = (id) => {
    setActive(id);
    window.dispatchEvent(new CustomEvent('gannzilla:right-drawing-tool-v126', { detail: { tool: id } }));
  };
  ...
  return (
    <div
      ...
      data-gannzilla-right-palette-v126="true"
      style={{
        position: 'fixed',
        right: 18,
        top,
