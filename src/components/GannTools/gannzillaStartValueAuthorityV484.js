const BUILD = 484;
const STATE_KEY = '__gannzillaStartValueAuthorityV484';
...
function commitPath(path, value, source = 'property') {
  ...
  try {
    ...
    url.searchParams.set('startValueControlsFirstCell', 'true');
    url.searchParams.set('v', String(BUILD));
    window.history.replaceState(window.history.state, '', `${url.pathname}${url.search}${url.hash}`);
  } catch (_) { /* URL persistence is optional */ }
  window.__gannzillaStartValueAuthorityLastCommitV484 = { path, value, source, at: Date.now() };
  return next;
}
...
function draw(source = 'refresh') {
  ...
  // Size means the exact number of visible numbered rings. The first cell is price.value.
  const totalRingCount = cfg.levels;
  ...
  for (let ring = 1; ring <= totalRingCount; ring += 1) {
    ...
    const fill = ring % 2 === 0 ? DARK_FILL : LIGHT_FILL;
    ...
    for (let index = 0; index < cfg.divisions; index += 1) {
      ...
      const value = cfg.startValue + (((ring - 1) * cfg.divisions + index) * cfg.increment);
      const displayValue = Number.isInteger(value) ? value : Number(value.toFixed(4));
      drawText(ctx, displayValue, point.x, point.y, scaledFontSize, maxWidth, cfg.fontWeight);
    }
  }

  ctx.restore();
  canvas.dataset.gannzillaStartValueAuthorityV484 = 'true';
  ...
  canvas.dataset.gannzillaFirstCellValue = String(cfg.startValue);
  canvas.dataset.gannzillaLastCellValue = String(cfg.startValue + ((cfg.levels * cfg.divisions - 1) * cfg.increment));
  ...
  lastDraw = {
    ...
    firstCellValue: cfg.startValue,
    lastCellValue: cfg.startValue + ((cfg.levels * cfg.divisions - 1) * cfg.increment),
  ...
  window.__gannzillaAllToolsRuntimeV482?.sched?.(`start-value-v${BUILD}-${source}`);
  window.dispatchEvent(new CustomEvent('gannzilla:start-value-authority-v484', {
    detail: { ...lastDraw, build: BUILD },
  }));
  return true;
}

function schedule(source = 'schedule', delays = [0, 30, 100, 260]) {
...
function install() {
  ...
  const onResize = () => schedule('resize', [0, 80, 240]);
  ...
  [0, 50, 140, 340, 760, 1600, 3200, 5200].forEach((delay) => {
    setTimeout(() => schedule(`boot-${delay}`, [0, 40, 160]), delay);
  });

  window.GANNZILLA_START_VALUE_AUTHORITY_V484 = true;
  window.__auditGannzillaStartValueAuthorityV484 = () => {
    const canvas = findWheel();
    const cfg = settings();
    const first = Number(canvas?.dataset?.gannzillaFirstCellValue);
    return {
      ok: canvas instanceof HTMLCanvasElement
        && canvas.dataset.gannzillaStartValueAuthorityV484 === 'true'
        && first === cfg.startValue,
      build: BUILD,
      firstCellEqualsPriceValue: first === cfg.startValue,
      firstCellValue: first,
      requestedStartValue: cfg.startValue,
      exactVisibleRingCount: Number(canvas?.dataset?.gannzillaUnlimitedLayerCount) === cfg.levels,
      levels: cfg.levels,
      divisions: cfg.divisions,
      increment: cfg.increment,
      clockwise: cfg.clockwise,
      drawCount,
      lastDraw,
      lastCommit: window.__gannzillaStartValueAuthorityLastCommitV484 || null,
    };
  };
