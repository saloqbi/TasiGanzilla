const BUILD = 500;
const STATE_KEY = '__gannzillaFinalWheelAuthorityV500';
...
function draw(source = 'refresh') {
  ...
  // Base 1..36 + blank separator + anchor cycle ending at 3600 + requested outer cycles.
  ...
  canvas.dataset.gannzillaFinalWheelAuthorityV500 = 'true';
  ...
  window.dispatchEvent(new CustomEvent('gannzilla:final-wheel-authority-v500', {
    detail: { ...lastDraw, build: BUILD },
  }));
  return true;
}
...
function install() {
  ...
  window.GANNZILLA_FINAL_WHEEL_AUTHORITY_V500 = true;
  window.GANNZILLA_ANCHOR_3600_TEN_OUTER_CYCLES_V500 = true;
  window.__auditGannzillaFinalWheelAuthorityV500 = () => {
    const canvas = findWheel();
    const cfg = settings();
    ...
    return {
      ...
      firstOuterCycle: { start: cfg.anchorValue + cfg.increment, end: cfg.anchorValue + cfg.divisions * cfg.increment },
      finalOuterCycle: {
