...
function drawRotatedText(ctx, text, x, y, angleDegrees, fontSize, color = '#111111', weight = 700) {
  ctx.save();
...
function fittedFontSize(midRadius, ringWidth, divisions, textLength) {
  const arcWidth = (TWO_PI * midRadius) / divisions;
  const charFactor = textLength >= 6 ? 0.70 : textLength === 5 ? 0.64 : textLength === 4 ? 0.59 : 0.55;
  const byArc = (arcWidth * 0.72) / Math.max(1, textLength * charFactor);
  const byHeight = ringWidth * 0.31;
  return clamp(Math.min(byArc, byHeight), 10, 25);
}
...
function TopToolbar({ zoom, onZoomChange, onFit, clockwise, onDirectionChange }) {
  const iconStyle = {
    ...buttonStyle,
    width: 22,
    padding: 0,
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 8 }}>
        {['↖', '—', '▢', 'T', '🔒', '⌕'].map((label) => (
          <button type="button" key={label} style={iconStyle}>{label}</button>
        ))}
        <button type="button" onClick={onFit} style={iconStyle}>⛶</button>
        <button type="button" onClick={() => onZoomChange(clamp(zoom - (zoom > 1 ? 0.10 : 0.05), MIN_ZOOM, MAX_ZOOM))} style={iconStyle}>−</button>
        <span style={{ minWidth: 44, textAlign: 'center', fontWeight: 700 }}>{Math.round(zoom * 100)}%</span>
        <button type="button" onClick={() => onZoomChange(clamp(zoom + (zoom >= 1 ? 0.10 : 0.05), MIN_ZOOM, MAX_ZOOM))} style={iconStyle}>＋</button>
        <button type="button" onClick={() => onZoomChange(1)} style={{ ...buttonStyle, marginLeft: 5 }}>100%</button>
        <button type="button" onClick={() => onDirectionChange(!clockwise)} style={{ ...buttonStyle, marginLeft: 5 }}>{clockwise ? 'Clockwise' : 'Counter'}</button>
...
function SideShapeBar() {
  ...
  return (
    <div style={{ position: 'fixed', right: 18, top: 168, zIndex: 18, background: 'rgba(255,255,255,0.82)', border: '1px solid #e2e2e2', borderRadius: 18, padding: 7, display: 'flex', flexDirection: 'column', gap: 10 }}>
      {shapes.map((shape) => (
        <button type="button" key={shape} style={{ width: 28, height: 28, border: '1px solid #d2d2d2', borderRadius: 3, background: '#fafafa', color: '#9aa', fontSize: 18 }}>{shape}</button>
      ))}
    </div>
  );
}

export default function GannzillaClassicExactV92() {
  ...
  const [startValue, setStartValue] = useState(() => numberParam('startValue', 1, -100000000, 100000000));
  ...
  const geometry = useMemo(() => {
    ...
    const outerRadius = wheelRadius
      + protractorGap
      + (showProtractor ? protractorWidth : 0)
      + chronometerGap
      + (showChronometer ? chronometerWidth : 0)
      + 72;
  ...
  const fitToScreen = React.useCallback(() => {
    const viewport = viewportRef.current;
    if (!viewport) return;
    const fit = Math.min((viewport.clientWidth - 70) / geometry.size, (viewport.clientHeight - 70) / geometry.size);
  ...
  useEffect(() => {
    ...
    for (let ring = 1; ring <= levels; ring += 1) {
      ...
      const fill = ring % 2 === 0 ? '#e4e4e1' : '#ffffff';
      ...
      for (let index = 0; index < divisions; index += 1) {
        ...
        if (showNumbers) {
          ...
          const fontSize = fittedFontSize(mid, geometry.ringWidth, divisions, label.length);
          drawRotatedText(ctx, label, point.x, point.y, centerDegrees, fontSize, '#111111', 700);
        }
      }
    }
    ...
    if (showProtractor) {
      ...
      for (let degree = 0; degree < 360; degree += 5) {
        ...
        const point1 = polar(cx, cy, major ? inner - 2 : inner, direction * degree);
        const point2 = polar(cx, cy, outer + (major ? 5 : 0), direction * degree);
        ctx.beginPath();
        ...
        ctx.stroke();
        if (major && showMarks) {
          const labelPoint = polar(cx, cy, outer + 22, direction * degree);
          drawRotatedText(ctx, `${degree}°`, labelPoint.x, labelPoint.y, direction * degree, 14, '#666', 700);
        }
      }
      ctx.restore();
    }
    ...
    if (showChronometer) {
      ...
      labels.forEach((label, index) => {
        ...
        drawRotatedText(ctx, label, point.x, point.y, degree, 13, '#777', 700);
      });
      ctx.restore();
    }
    ...
    const pointer = polar(cx, cy, geometry.wheelRadius + 80, pointerDegrees);
    ctx.save();
    ...
    ctx.fill();
    ctx.restore();
  }, [geometry, zoom, levels, divisions, startValue, increment, clockwise, layoutVisible, showNumbers, showMarks, showProtractor, showChronometer, showCosmogram, highlightVisible, highlightFill, findValue, angle]);
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', overflow: 'hidden', background: '#ffffff', direction: 'ltr' }}>
      <TopToolbar zoom={zoom} onZoomChange={(value) => { setZoom(value); window.setTimeout(centerViewport, 30); }} onFit={fitToScreen} clockwise={clockwise} onDirectionChange={setClockwise} />
      ...
      {panelVisible && (
        <aside style={panelStyle}>
          ...
          <Section title="Chronometer" icon="◷">
            ...
            <CheckField label="Clockwise" checked={clockwise} onChange={setClockwise} />
            <Field label="Angle"><input style={fieldStyle} type="number" value={chronoAngle} onChange={(event) => setChronoAngle(Number(event.target.value) || 0)} /></Field>
