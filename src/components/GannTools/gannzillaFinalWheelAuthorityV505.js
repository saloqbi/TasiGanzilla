const BUILD = 505;
const STATE_KEY = '__gannzillaFinalWheelAuthorityV505';
...
function draw(source = 'refresh') {
  ...
  canvas.dataset.gannzillaFinalWheelAuthorityV505 = 'true';
  ...
  window.dispatchEvent(new CustomEvent('gannzilla:final-wheel-authority-v505', {
    detail: { ...lastDraw, build: BUILD },
  }));
  return true;
}
...
function install() {
  ...
  window.GANNZILLA_FINAL_WHEEL_AUTHORITY_V505 = true;
  window.GANNZILLA_UNIFIED_28PX_TYPOGRAPHY_V505 = true;
  window.__auditGannzillaFinalWheelAuthorityV505 = () => {
    const canvas = findWheel();
    const cfg = settings();
    const dpr = devicePixelRatioValue();
