...
function readState() {
  try {
    const runtime = window.__gannzillaReferencePanelStateV421 || window.__gannzillaCanonicalPanelStateV326;
...
function findWheel() {
  ...
  return Array.from(document.querySelectorAll('canvas'))
    .filter((canvas) => {
      if (!(canvas instanceof HTMLCanvasElement) || canvas.closest('aside')
        || canvas.id === DRAWING_OVERLAY_ID || canvas.id === THEME_OVERLAY_ID) return false;
...
function draw(source = 'refresh') {
  ...
  for (let ring = 1; ring <= totalRingCount; ring += 1) {
    ...
    for (let index = 0; index < cfg.divisions; index += 1) {
      ...
      const applied = drawNumber(ctx, displayValue, point.x, point.y, cfg.numberFontSize, maxWidth, effectiveRingWidth, cfg.numberWeight);
  ...
  drawCount += 1;
  lastDraw = {
    source, levels: cfg.levels, divisions: cfg.divisions, clockwise: cfg.clockwise,
    totalRingCount, originalInnerRadius: cfg.originalInnerRadius, adjustedInnerRadius,
    configuredRingWidth: cfg.ringWidth, effectiveRingWidth, numberFontSize: cfg.numberFontSize,
  ...
  window.dispatchEvent(new CustomEvent('gannzilla:final-wheel-authority-v491', { detail: { ...lastDraw, build: BUILD } }));
  return true;
}
...
function schedule(source = 'schedule', delays = [0, 60, 180, 420, 900]) {
  ...
  delays.filter(Boolean).forEach((delay) => {
    ...
    timers.set(delay, setTimeout(() => { timers.delete(delay); draw(`${source}-${delay}`); }, delay));
  });
}
...
function persistFlags() {
  ...
  } catch (_) { /* Runtime rendering remains authoritative. */ }
}
...
function install() {
  ...
  const observer = new MutationObserver((records) => {
    const canvas = findWheel();
    if (records.some((record) => record.target !== canvas && !canvas?.contains?.(record.target))) schedule('mutation', [0, 100, 360]);
  });
