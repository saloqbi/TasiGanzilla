...
const BUILD = 591;
const STATE_KEY = '__gannzillaCenterLogoStageV591';
const IMAGE_ID = 'gannzilla-center-logo-stage-v591';
const STYLE_ID = 'gannzilla-center-logo-stage-style-v591';
const STAGE_DATASET_KEY = 'gannzillaCenterLogoStageV591';
...
function ensureStyle() {
  ...
  style.textContent = `
    [data-gannzilla-center-logo-stage-v591="true"] {
...
function ensureImage(stage) {
  ...
  if (!(image instanceof HTMLImageElement)) {
    ...
    image.addEventListener('error', () => {
      image.style.setProperty('visibility', 'hidden', 'important');
      image.style.setProperty('opacity', '0', 'important');
      image.dataset.gannzillaCenterLogoImageErrorV591 = 'true';
    }, { once: true });
  }
...
function apply(source = 'apply') {
  ...
  image.dataset.gannzillaCenterLogoStageV591 = 'true';
  image.dataset.gannzillaCenterLogoBuildV591 = String(BUILD);
  image.dataset.gannzillaCenterLogoScaleV591 = String(LOGO_SCALE);
  image.dataset.gannzillaCenterLogoViewportScaleV591 = viewportScale.toFixed(6);
  image.dataset.gannzillaCenterLogoEmptyRadiusV591 = emptyRadiusOnScreen.toFixed(3);
  image.dataset.gannzillaCenterLogoDiameterV591 = diameter.toFixed(3);
  image.dataset.gannzillaCenterLogoInteractiveV591 = 'false';
  image.dataset.gannzillaCenterLogoStageBoundV591 = 'true';
  image.dataset.gannzillaCenterLogoPanAddedV591 = 'false';
...
function install() {
  ...
  window.GANNZILLA_CENTER_LOGO_STAGE_V591 = true;
  window.__auditGannzillaCenterLogoStageV591 = () => {
    const image = document.getElementById(IMAGE_ID);
    const canvas = findWheel();
    const stage = image?.parentElement;
    const imageRect = image?.getBoundingClientRect();
    const emptyRadius = Number(image?.dataset.gannzillaCenterLogoEmptyRadiusV591 || 0);
    const logoDiameter = Number(image?.dataset.gannzillaCenterLogoDiameterV591 || 0);
    return {
