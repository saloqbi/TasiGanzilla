const BUILD = 503;
const STATE_KEY = '__gannzillaFinalWheelAuthorityV503';
...
const MAX_RENDER_PIXEL_DIMENSION = 8192;
const DEFAULT_PAINT_RENDER_SCALE = 3;
...
function settings() {
  return {
    ...
    paintCrispMode: booleanSetting('display.paintCrispMode', 'paintCrispMode', true),
    paintRenderScale: numericSetting(
      'display.paintRenderScale',
      'paintRenderScale',
      DEFAULT_PAINT_RENDER_SCALE,
      1,
      4,
    ),
  };
...
function effectiveRenderScale(logicalSize, cfg) {
  const nativeDpr = clamp(Number(window.devicePixelRatio) || 1, 1, 2);
  const requested = cfg.paintCrispMode
    ? Math.max(nativeDpr, cfg.paintRenderScale)
    : nativeDpr;
  const dimensionCap = Math.max(1, MAX_RENDER_PIXEL_DIMENSION / Math.max(1, logicalSize));
  return Number(clamp(Math.min(requested, dimensionCap), 1, 4).toFixed(4));
}

function drawNumber(ctx, value, x, y, renderScale, options = {}) {
  const fontSize = options.fontSize || NUMBER_FONT_SIZE;
  const fontWeight = options.fontWeight || NUMBER_FONT_WEIGHT;
  ctx.save();
  ctx.translate(
    Math.round(x * renderScale) / renderScale,
    Math.round(y * renderScale) / renderScale,
  );
...
function draw(source = 'refresh') {
  ...
  const renderScale = effectiveRenderScale(logicalSize, cfg);
  const previousRenderScale = Number(canvas.dataset.gannzillaEffectiveRenderScale)
    || clamp(Number(window.devicePixelRatio) || 1, 1, 2);
  const previousLogicalWidth = Math.max(1, canvas.width / previousRenderScale);
  const rect = canvas.getBoundingClientRect();
  const visualScale = rect.width > 0 ? rect.width / previousLogicalWidth : 1;
  const pixelWidth = Math.ceil(logicalSize * renderScale);
  const pixelHeight = Math.ceil(logicalSize * renderScale);
  ...
  const cssSize = Math.max(1, Math.round(logicalSize * visualScale));
  ...
  for (let ring = 1; ring <= totalRingCount; ring += 1) {
    ...
    for (let index = 0; index < cfg.divisions; index += 1) {
      ...
      if (ring === 2) {
        ...
        drawNumber(ctx, rootValue, point.x, point.y, renderScale, {
      ...
      drawNumber(ctx, displayValue, point.x, point.y, renderScale);
    }
  }
  ...
  const requestedScaleReached = renderScale >= Math.min(
    cfg.paintRenderScale,
    MAX_RENDER_PIXEL_DIMENSION / logicalSize,
  ) - 0.01;

  canvas.dataset.gannzillaFinalWheelAuthorityV503 = 'true';
  ...
  canvas.dataset.gannzillaPaintCrispMode = String(cfg.paintCrispMode);
  canvas.dataset.gannzillaRequestedRenderScale = String(cfg.paintRenderScale);
  canvas.dataset.gannzillaEffectiveRenderScale = String(renderScale);
  canvas.dataset.gannzillaLogicalWidth = String(logicalSize);
  canvas.dataset.gannzillaLogicalHeight = String(logicalSize);
  canvas.dataset.gannzillaCanvasPixelWidth = String(canvas.width);
  canvas.dataset.gannzillaCanvasPixelHeight = String(canvas.height);
  canvas.dataset.gannzillaPaintScaleReached = String(requestedScaleReached);
  ...
  lastDraw = {
    ...
    paintCrispMode: cfg.paintCrispMode,
    requestedRenderScale: cfg.paintRenderScale,
    effectiveRenderScale: renderScale,
    logicalSize,
    canvasPixelSize: canvas.width,
    requestedScaleReached,
    safetyScaled,
    at: Date.now(),
  };
  ...
  window.dispatchEvent(new CustomEvent('gannzilla:final-wheel-authority-v503', {
    detail: { ...lastDraw, build: BUILD },
  }));
  return true;
}
...
function persistFlags() {
  try {
    ...
    url.searchParams.set(
      'paintRenderScale',
      url.searchParams.get('paintRenderScale') || String(DEFAULT_PAINT_RENDER_SCALE),
    );
...
function install() {
  ...
  window.GANNZILLA_FINAL_WHEEL_AUTHORITY_V503 = true;
  window.GANNZILLA_PAINT_CRISP_RENDERING_V503 = true;
  window.__auditGannzillaFinalWheelAuthorityV503 = () => {
    const canvas = findWheel();
    const cfg = settings();
    const expectedNorth = Array.from({ length: cfg.levels + 1 }, (_, ringIndex) =>
      cfg.anchorValue + ringIndex * cfg.divisions * cfg.increment);
    const expectedFinalNorth = cfg.anchorValue + cfg.levels * cfg.divisions * cfg.increment;
    const effectiveScale = Number(canvas?.dataset?.gannzillaEffectiveRenderScale || 0);
    return {
      ok: canvas instanceof HTMLCanvasElement
        ...
        && effectiveScale > 1
        && canvas.dataset.gannzillaPaintScaleReached === 'true',
      build: BUILD,
      anchorValue: cfg.anchorValue,
      ...
      paintDisplay: {
        mode: cfg.paintCrispMode,
        requestedRenderScale: cfg.paintRenderScale,
        effectiveRenderScale: effectiveScale,
        logicalWidth: Number(canvas?.dataset?.gannzillaLogicalWidth || 0),
        canvasPixelWidth: canvas?.width || 0,
        requestedScaleReached: canvas?.dataset?.gannzillaPaintScaleReached === 'true',
      },
      blueNumberColor: BLUE_NUMBER_COLOR,
