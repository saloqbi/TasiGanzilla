...
// المكونات المالية البسيطة للتداول
const GannMarketOverlay = ({ center, radius, selectedSymbol, selectedMarket, currentPrice, gannAngle, ctx, settings }) => {
  return null; // مكون مؤقت للتطوير المستقبلي
};

const MarketDashboard = ({ onSymbolSelect, selectedSymbol }) => {
  return (
    <div style={{
      background: '#1a1a1a',
      ...
      <h3 style={{ margin: '0 0 10px 0', fontSize: '16px' }}>لوحة الأسواق المالية</h3>
      <p style={{ margin: '5px 0', fontSize: '14px' }}>سيتم إضافة بيانات السوق هنا</p>
      <select 
        onChange={(e) => onSymbolSelect && onSymbolSelect(e.target.value)}
        style={{
          background: '#333',
          color: '#FFD700',
...
const TradingSignals = ({ selectedSymbol, selectedMarket, currentPrice, gannAngle }) => {
  return (
    <div style={{
      background: '#1a1a1a',
      border: '2px solid #FFD700',
      ...
      <h3 style={{ margin: '0 0 10px 0', fontSize: '16px' }}>إشارات التداول</h3>
      <p style={{ margin: '5px 0', fontSize: '14px' }}>الرمز: {selectedSymbol || 'غير محدد'}</p>
      <p style={{ margin: '5px 0', fontSize: '14px' }}>السعر: {currentPrice || 'غير متوفر'}</p>
      <p style={{ margin: '5px 0', fontSize: '14px' }}>زاوية جان: {gannAngle || 'غير محسوبة'}</p>
    </div>
  );
};

// GannCircle360CanvasLifeTrading Component - نسخة محدثة للتداول
...
const GannCircle360CanvasLifeTrading = () => {
  ...
  const [gannAngle, setGannAngle] = useState(null);
  const [showMarketOverlay, setShowMarketOverlay] = useState(false);
  const [marketIntegrationMode, setMarketIntegrationMode] = useState(false);
...
const [selectedPatternIndex, setSelectedPatternIndex] = useState(null);
  ...
  // دالة لمعالجة اختيار الرموز المالية
  const handleSymbolSelect = (symbol, market, price, angle) => {
    ...
    // تحديث قيمة البداية في دائرة Gann لتتطابق مع السعر
    if (price) {
      setSettings(prev => ({
        ...prev,
        startValue: parseFloat(price)
      }));
    }
  };
    ...
    >
      ...
      <div style={{ padding: 10, flexShrink: 0 }}>
        ...
        <div style={{
          ...
          {/* زر وضع التداول */}
          <button
            onClick={() => setMarketIntegrationMode((prev) => !prev)}
            style={{
              position: "fixed",
              top: "150px",
              left: "20px",
              ...
              height: "25px",
              borderRadius: "50%",
              backgroundColor: marketIntegrationMode ? "#00ff00" : "#ffcc00",
              color: "#000",
              fontSize: "10px",
              border: "none",
              cursor: "pointer",
            ...
            title={marketIntegrationMode ? "إيقاف وضع التداول" : "تفعيل وضع التداول"}
          >
            💹
          </button>
          ...
          {showSettings && (
            <>
              ...
              {/* أزرار الأسواق المالية */}
              {marketIntegrationMode && (
                <>
                  <button
                    onClick={() => setShowMarketDashboard((v) => !v)}
                    style={buttonStyle}
                  >
                    💹 {settings.language === "ar"
                      ? (showMarketDashboard ? "إخفاء لوحة السوق" : "إظهار لوحة السوق")
                      : (showMarketDashboard ? "Hide Market Dashboard" : "Show Market Dashboard")}
                  </button>
                  <button
                    onClick={() => setShowMarketOverlay((v) => !v)}
                    style={buttonStyle}
                  >
                    📊 {settings.language === "ar"
                      ? (showMarketOverlay ? "إخفاء طبقة السوق" : "إظهار طبقة السوق")
                      : (showMarketOverlay ? "Hide Market Overlay" : "Show Market Overlay")}
                  </button>
                </>
              )}
...
{showRepeatedPattern && (
  <>
    ...
    >
      ...
      {/* إضافة overlay للأسواق المالية */}
      {marketIntegrationMode && showMarketOverlay && (
        <GannMarketOverlay
          center={canvasSize / 2}
          radius={actualMaxRadius}
      ...
      {/* لوحة الأسواق المالية */}
      {marketIntegrationMode && showMarketDashboard && (
        <div style={{
          position: 'fixed',
          top: '20px',
          right: '20px',
      ...
      {/* إشارات التداول */}
      {marketIntegrationMode && showTradingSignals && (
        <div style={{
          position: 'fixed',
          bottom: '20px',
          left: '20px',
      ...
      {showMarketSettings && (
        <div style={{
          ...
          <h3>إعدادات السوق المالي</h3>
          <p>سيتم إضافة إعدادات السوق المالي هنا</p>
          <button 
            onClick={() => setShowMarketSettings(false)}
            style={{
              background: '#333',
              color: '#FFD700',
              border: '1px solid #FFD700',
