...
function drawComfortProtractor() {
  ...
  const protractorWidth = numberParam('gannzillaProtractorOuterWidth', 128, 72, 150);
  const majorFontSize = numberParam('gannzillaProtractorFontSize', 20, 16, 30);
  const fiveFontSize = numberParam('gannzillaFiveDegreeFontSize', 14, 10, 20);
  ...
  const separator = inner + (protractorWidth * 0.50);
  const fiveLabelRadius = inner + (protractorWidth * 0.27);
  const tenLabelRadius = inner + (protractorWidth * 0.73);
  const eraseOuter = Math.min(Math.min(width, height) / 2 - 2, outer + 32);
  const cx = width / 2;
  const cy = height / 2;
  const ctx = canvas.getContext('2d');
  if (!ctx || eraseOuter <= inner) return false;
  ...
  // Remove the original narrow protractor and its old labels before drawing one enlarged owner.
  fillAnnulus(ctx, cx, cy, Math.max(0, inner - 5), eraseOuter, '#ffffff');

  ctx.strokeStyle = COLORS.border;
  ctx.lineWidth = 1.15;
  [inner, separator, outer].forEach((radius) => {
    ctx.beginPath();
  ...
  for (let degree = 0; degree < 360; degree += 5) {
    ...
    ctx.strokeStyle = major ? COLORS.majorLine : (ten ? COLORS.tenLine : COLORS.minorLine);
  ...
  React.useLayoutEffect(() => {
    ...
    const schedule = (delay = 0) => {
      const timer = window.setTimeout(() => {
        timers.delete(timer);
        if (!disposed) window.requestAnimationFrame(drawComfortProtractor);
      }, delay);
      timers.add(timer);
    };

    const scheduleAfterChange = () => {
      [20, 120, 280, 620].forEach(schedule);
    };
    ...
    window.GANNZILLA_COMFORT_PROTRACTOR_V378 = true;
    window.__auditGannzillaComfortProtractorV378 = () => ({
      ok: Boolean(findWheelCanvas()?.dataset?.gannzillaComfortProtractorV378 === 'true'),
      build: BUILD,
      actualOuterBandWidthPx: numberParam('gannzillaProtractorOuterWidth', 128, 72, 150),
      originalOuterBandWidthPx: 34,
      oldSupplementalAngleOwnerRemoved: true,
      singleProtractorOwner: true,
    ...
    return () => {
      ...
      delete window.GANNZILLA_COMFORT_PROTRACTOR_V378;
      delete window.__auditGannzillaComfortProtractorV378;
    };
  }, []);
