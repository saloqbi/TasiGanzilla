...
const MARKER = 'GANNZILLA_NATIVE_LEFT_PALETTE_V113_NATIVE_TOGGLE';
...
const STORAGE_KEY = 'gannzillaDrawingPaletteVisibleV113Native';
...
function styleToggleHost(button, visible) {
  button.type = 'button';
...
function ToggleIcon({ visible }) {
  return (
    <svg width="17" height="17" viewBox="0 0 24 24" aria-hidden="true" style={{ display: 'block', pointerEvents: 'none' }}>
      <rect x="4" y="3" width="16" height="18" rx="1" fill="#f7f7f7" stroke="#5f7f95" strokeWidth="1.35" />
      <rect x="6.5" y="5" width="4" height="14" fill="#b9ddf4" stroke="#4f8cb4" strokeWidth="0.9" />
      <path d="M12.5 6.5h4.6M12.5 11.8h4.6M12.5 17.1h4.6" stroke="#718895" strokeWidth="1.25" strokeLinecap="round" />
      <circle cx="18.5" cy="5.1" r="2.2" fill={visible ? '#22884f' : '#969696'} />
      {visible ? (
        <path d="m17.3 5.1.9 1 1.6-1.9" fill="none" stroke="#fff" strokeWidth="0.85" strokeLinecap="round" strokeLinejoin="round" />
      ) : (
        <path d="M17.25 5.1h2.5" stroke="#fff" strokeWidth="0.85" strokeLinecap="round" />
      )}
    </svg>
  );
}
...
export default function GannzillaNativeLeftPaletteV110() {
  ...
  const [toggleHost, setToggleHost] = React.useState(null);
  ...
  React.useEffect(() => {
    ...
    const bindToggleHost = () => {
      const toolbar = findTopToolbar();
      const lockButton = findLockButton(toolbar);
      if (!lockButton?.parentElement) return;

      let host = document.getElementById(TOGGLE_BUTTON_ID);
      if (!host) {
        host = document.createElement('button');
        host.id = TOGGLE_BUTTON_ID;
      }

      const parent = lockButton.parentElement;
      if (lockButton.nextElementSibling !== host) {
        parent.insertBefore(host, lockButton.nextElementSibling);
      }
      ...
      setToggleHost((current) => (current === host ? current : host));
    };
    ...
    window[MARKER] = true;
    window.__auditGannzillaNativeLeftPaletteV113NativeToggle = () => ({
      ok: window[MARKER] === true
        && Boolean(document.getElementById(TOGGLE_BUTTON_ID))
        && Boolean(document.getElementById(PALETTE_ID) || visible === false),
      toggleButtonPresent: Boolean(document.getElementById(TOGGLE_BUTTON_ID)),
      paletteVisible: visible,
      palettePosition: 'outside-settings-panel-with-gap',
      wheelUntouched: true,
    });
    ...
    return () => {
      ...
      const host = document.getElementById(TOGGLE_BUTTON_ID);
      if (host) {
        host.onclick = null;
        host.remove();
      }
  ...
  React.useEffect(() => {
    if (toggleHost) styleToggleHost(toggleHost, visible);
  ...
  return (
    <>
      {toggleHost && createPortal(<ToggleIcon visible={visible} />, toggleHost)}
