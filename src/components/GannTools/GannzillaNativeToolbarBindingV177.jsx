...
const BUILD = '177';
const DRAW_LAYER_ID = 'gannzilla-native-binding-draw-layer-v177';
const STATUS_ID = 'gannzilla-native-binding-status-v177';
const TOOL_LABELS = {
  '↖': 'select',
  '—': 'line',
  '▢': 'rectangle',
...
function applyPalettesVisible(visible) {
  ...
  const detail = { visible, source: `native-binding-v${BUILD}` };
...
export default function GannzillaNativeToolbarBindingV177() {
  ...
  const palettesVisibleRef = React.useRef(palettesVisible);
  ...
  React.useEffect(() => { palettesVisibleRef.current = palettesVisible; }, [palettesVisible]);
  ...
  const activate = React.useCallback((tool) => {
    ...
    if (tool === 'palettes') {
      setPalettesVisible((current) => {
        const next = !current;
        palettesVisibleRef.current = next;
        applyPalettesVisible(next);
        flash(next ? 'تم إظهار أدوات الرسم' : 'تم إخفاء أدوات الرسم');
        return next;
      });
      return;
    }
    ...
    const sync = () => {
      ...
      document.querySelectorAll('button').forEach((button) => {
        ...
        const active = (['select', 'line', 'rectangle', 'text'].includes(tool) && activeToolRef.current === tool)
          ...
          || (tool === 'palettes' && palettesVisibleRef.current)
      ...
      const legacyHitbox = document.getElementById('gannzilla-drawing-tools-hitbox-v125');
      if (legacyHitbox) {
        legacyHitbox.style.setProperty('pointer-events', 'auto', 'important');
      }
    };
  ...
  React.useEffect(() => {
    window.GANNZILLA_NATIVE_TOOLBAR_BINDING_V177 = true;
    window.__auditGannzillaNativeToolbarBindingV177 = () => ({
      ok: true,
      build: BUILD,
      activeTool,
      locked,
  ...
  }, [activeTool, locked, palettesVisible, shapes.length]);
